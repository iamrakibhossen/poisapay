 ...
 *  - `shop_sales_pages.draft`  — the working document edited in the builder. The
 ...
 *  - `shop_page_revisions`     — version history (snapshot on publish + autosave
 *    checkpoints); restore loads a revision back into `draft`.
 *  - `shop_saved_blocks`       — a seller's reusable saved sections/components.
 */
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('shop_sales_pages', function (Blueprint $table) {
    ...
    {
        Schema::dropIfExists('shop_saved_blocks');
        Schema::dropIfExists('shop_page_revisions');
        Schema::table('shop_sales_pages', function (Blueprint $table) {
