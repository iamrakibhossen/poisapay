...
return new class extends Migration
{
    ...
    {
        // ── Download grants ─────────────────────────────────────────────────────
        Schema::create('sell_downloads', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->foreignUuid('order_item_id')->constrained('sell_order_items')->cascadeOnDelete();
            $table->foreignUuid('product_file_id')->constrained('sell_product_files');
        ...
        Schema::create('sell_download_events', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->foreignUuid('download_id')->constrained('sell_downloads')->cascadeOnDelete();
        ...
        Schema::create('sell_licenses', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->foreignUuid('product_id')->constrained('sell_products')->cascadeOnDelete();
            $table->foreignUuid('order_item_id')->nullable()->constrained('sell_order_items')->nullOnDelete();
        ...
        DB::statement("CREATE INDEX sell_licenses_pool ON sell_licenses (product_id) WHERE status = 'available'");
        ...
        Schema::create('sell_reviews', function (Blueprint $table) {
            ...
            $table->foreignUuid('product_id')->constrained('sell_products')->cascadeOnDelete();
            $table->foreignUuid('order_id')->constrained('sell_orders')->cascadeOnDelete(); // proves purchase
            $table->foreignUuid('buyer_user_id')->nullable()->constrained('users')->nullOnDelete();
        ...
        // on sell_orders (see the orders migration): fewer joins, at the cost of the
        // order row taking the messaging write.
        Schema::create('sell_messages', function (Blueprint $table) {
            $table->uuid('id')->primary();
        ...
        Schema::create('sell_message_attachments', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->foreignUuid('message_id')->constrained('sell_messages')->cascadeOnDelete();
    ...
    {
        Schema::dropIfExists('sell_message_attachments');
        Schema::dropIfExists('sell_messages');
        Schema::dropIfExists('sell_reviews');
        Schema::dropIfExists('sell_licenses');
        Schema::dropIfExists('sell_download_events');
        Schema::dropIfExists('sell_downloads');
    }
};
