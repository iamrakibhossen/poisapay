...
beforeEach(function () {
    // The Exchange is crypto-only, so this suite swaps a crypto stablecoin (USDC,
    // 2 decimals, priced $1 by StubRateProvider) into USDT — keeping the clean
    // $1:$1 math the fiat-based version relied on.
    $this->usdt = testAsset('USDT', 6, 'tron');
    $this->usdc = testAsset('USDC', 2, 'tron');
...
it('with fee_bps=0 books spread only and moves both balances (regression parity)', function () {
    ...
    creditUser($this->user, $this->usdc, '10000'); // $100.00

    $quote = quoteHundred();
    $this->action->execute($this->user, $quote);

    $resolver = app(AccountResolver::class);
    $spread = $resolver->system(LedgerAccountType::FxSpreadIncome, $this->usdc->id)->fresh('balance')->money()->baseString();
    $fee = $resolver->system(LedgerAccountType::FeeIncome, $this->usdc->id)->fresh('balance')->money()->baseString();

    // spread = 10000 * 75 / 10000 = 75 minor USDC; no platform fee.
    expect($spread)->toBe('75')
        ->and($fee)->toBe('0')
        ->and($this->ledger->availableBalance($this->user, $this->usdc->id)->baseString())->toBe('0')
...
it('enforces the minimum KYC tier for swaps', function () {
    ...
    creditUser($this->user, $this->usdc, '10000');
...
it('enforces the per-user daily swap notional limit', function () {
    ...
    $this->user = User::factory()->create();
    creditUser($this->user, $this->usdc, '30000'); // $300.00
...
it('writes a swap.completed audit row with before/after balances and rate source', function () {
    ...
    expect($log)->not->toBeNull()
        ->and($log->changes['before']['USDC'])->toBe('100.00')
        ->and($log->changes['after']['USDC'])->toBe('0.00')
...
it('exposes an idempotent swap API keyed by the Idempotency-Key header', function () {
    ...
    $quote = $this->postJson('/api/v1/swaps/quote', ['from' => 'USDC', 'to' => 'USDT', 'amount' => '100'])
