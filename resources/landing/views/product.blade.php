            ...
            <div class="relative flex justify-center lg:justify-end">
                ...
                <div class="relative w-full max-w-sm">
                    @switch($slug)
                        ...
                        @case('exchange')
                            @php
                                // Illustrative ETH→USDT swap — the Exchange is crypto-to-crypto only.
                                // ETH priced in USD ≈ USDT (a $1 stablecoin), via the live feed.
                                $cvQuote = 'USDT';
                                $cvRate = (float) (app(\App\Domain\Exchange\CoinGeckoRateProvider::class)->ratesWithFallback('USD', ['ETH'])['ETH'] ?? 0);
                            @endphp
                            {{-- Swap widget --}}
                            <div class="lp-card p-6">
                                ...
                                <div class="relative mt-4 space-y-2">
                                    ...
                                    <div class="rounded-2xl border border-slate-200 bg-white p-4">
                                        ...
                                        <div class="mt-1 flex items-center justify-between">
                                            ...
                                            <span class="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700">{{ $cvQuote }}</span>
                                        </div>
                                    </div>
                                </div>
                                <p class="mt-3 text-center text-xs text-slate-400">{{ __('Rate 1 ETH ≈ :rate :currency · spread shown up front', ['rate' => number_format($cvRate, 2), 'currency' => $cvQuote]) }}</p>
                            </div>
                            @break
                        ...
                        @case('p2p')
                            ...
                            <div class="lp-card p-6">
                                <div class="flex items-center justify-between">
                                    <p class="text-sm font-semibold text-slate-900">{{ __('Best offer · Buy USDT') }}</p>
                                    <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold" style="background:rgba(16,185,129,.12);color:var(--up)">
                                        <x-heroicon-s-lock-closed class="h-3 w-3" /> {{ __('Escrow') }}
                                    </span>
                                </div>
                                <div class="mt-4 flex items-center gap-3">
                                    <span class="grid h-11 w-11 flex-none place-items-center rounded-full text-sm font-bold text-white" style="background:linear-gradient(135deg,var(--brand),var(--brand-600))">R</span>
                                    <div class="min-w-0">
                                        <p class="flex items-center gap-1 text-sm font-bold text-slate-900">Rahman Store <x-heroicon-s-check-badge class="h-4 w-4" style="color:var(--brand)" /></p>
                                        <p class="flex items-center gap-1 text-xs text-slate-500"><x-heroicon-s-star class="h-3 w-3 text-amber-400" /> {{ __('1,204 trades · 99% completion') }}</p>
                                    </div>
                                </div>
                                <div class="mt-4 flex items-baseline justify-between">
                                    <span class="text-3xl font-bold tracking-tight text-slate-900">121.50 <span class="text-base font-semibold text-slate-400">BDT</span></span>
                                    <span class="text-xs text-slate-400">{{ __('per USDT') }}</span>
                                </div>
                                <div class="mt-3 space-y-1.5 text-xs text-slate-500">
                                    <div class="flex items-center justify-between"><span>{{ __('Available') }}</span><span class="font-semibold text-slate-700">8,500 USDT</span></div>
                                    <div class="flex items-center justify-between"><span>{{ __('Limit') }}</span><span class="font-semibold text-slate-700">৳2,000 – ৳120,000</span></div>
                                </div>
                                <div class="mt-3 flex flex-wrap gap-1.5">
                                    <span class="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-600">{{ __('Bank transfer') }}</span>
                                    <span class="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-600">{{ __('bKash') }}</span>
                                </div>
                                <button type="button" class="mt-4 w-full rounded-xl py-3 text-sm font-semibold text-white shadow-sm" style="background:var(--brand)">{{ __('Buy USDT') }}</button>
                            </div>
                            @break
                        ...
                        @case('merchant-pay')
                            ...
                            <div class="lp-card p-6 text-center">
                                ...
                                <p class="mt-1 text-3xl font-bold tracking-tight text-slate-900">$50.00 <span class="text-base font-semibold text-slate-400">USD</span></p>
