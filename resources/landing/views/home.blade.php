...
<x-landing::layouts.master main-class="flex-1" :seo="$seo">
    ...
    <section id="exchange" class="relative overflow-hidden px-4 pb-16 pt-32 sm:px-6 lg:px-8 lg:pb-24 lg:pt-40">
        <div class="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1.05fr_1fr] lg:gap-8">
            ...
            <div class="text-center lg:text-left">
                <span class="lp-reveal inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 text-xs font-medium text-slate-700 lp-chip">
                    <span class="h-1.5 w-1.5 rounded-full lp-pulse" style="background:var(--brand)"></span>
                    {{ __('Now live · Tron-first custody') }}
                </span>

                <h1 class="lp-reveal mx-auto mt-6 max-w-2xl text-[2.7rem] font-extrabold leading-[1.03] tracking-tight text-slate-900 sm:text-6xl lg:mx-0 lg:text-[4.2rem]" data-d="1">
                    {{ __('Spend crypto') }} <br class="hidden sm:block" />{{ __('like') }} <span class="lp-grad-anim">{{ __('cash.') }}</span>
                </h1>

                <p class="lp-reveal mx-auto mt-6 max-w-xl text-lg leading-relaxed text-slate-600 lg:mx-0" data-d="2">
                ...
                <div class="lp-reveal mt-9 flex flex-wrap items-center justify-center gap-3 lg:justify-start" data-d="3">
                    <a href="{{ route('register') }}" class="lp-btn lp-btn-primary lp-btn-lg">
                        {{ __('Get started') }} <x-heroicon-o-arrow-right class="h-5 w-5" />
                    </a>
                    <a href="#wallet" class="lp-btn lp-btn-ghost lp-btn-lg">
                        <x-heroicon-s-play class="h-4 w-4" /> {{ __('View demo') }}
                    </a>
                </div>

                {{-- Trust badges --}}
                <div class="lp-reveal mt-10 flex flex-wrap items-center justify-center gap-x-6 gap-y-3 lg:justify-start" data-d="4">
            ...
            <div class="relative mx-auto w-full max-w-md">
                <div class="lp-reveal relative z-10" data-d="1">
                    <x-landing::converter id="hero" />
                </div>
            </div>
        </div>

        {{-- Logo marquee --}}
        <div class="lp-reveal lp-marquee-mask mx-auto mt-20 max-w-6xl overflow-hidden">
    ...
    <section id="prices" class="relative px-4 py-20 sm:px-6 lg:px-8 lg:py-24">
        @php
            ...
            $priceBase = \App\Support\BaseCurrency::displayCode();
            $priceSymbol = \App\Support\BaseCurrency::symbol($priceBase);
            $liveRates = app(\App\Domain\Exchange\CoinGeckoRateProvider::class)->ratesWithFallback($priceBase, $priceCoins);
        @endphp
        <div class="mx-auto max-w-6xl">
            <div class="mx-auto max-w-xl text-center lp-reveal">
                <p class="text-sm font-semibold uppercase tracking-[0.16em]" style="color:var(--brand)">{{ __('Live prices') }}</p>
                <h2 class="mt-3 text-3xl font-extrabold tracking-tight text-slate-900 sm:text-4xl">{{ __('Crypto prices, live in :currency', ['currency' => $priceBase]) }}</h2>
                <p class="mt-3 text-slate-600">{{ __('Real-time reference rates, refreshed continuously.') }}</p>
            </div>

            <div class="lp-reveal mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3" data-price-grid data-rates-url="{{ route('marketing.rates') }}" data-fiat-symbol="{{ $priceSymbol }}">
                @foreach ($priceCoins as $sym)
                    <a href="{{ route('marketing.prices') }}" class="lp-glass lp-card-hover flex items-center gap-4 rounded-2xl p-5">
                        ...
                        <div class="shrink-0 text-right">
                            <p class="lp-tabular text-base font-bold text-slate-900" data-price data-sym="{{ $sym }}">{{ $priceSymbol }}{{ number_format((float) ($liveRates[$sym] ?? 0), 2) }}</p>
                            <p class="text-[11px] font-medium uppercase tracking-wide text-slate-400">{{ $priceBase }}</p>
                        </div>
                    </a>
                @endforeach
            </div>
            ...
            <p class="mt-5 flex items-center justify-center gap-1.5 text-xs font-medium text-slate-400">
                ...
                <a href="{{ route('marketing.prices') }}" class="text-slate-500 underline-offset-2 hover:text-slate-900 hover:underline">{{ __('View all prices') }}</a>
            </p>
        </div>
    </section>
    ...
    <section id="cards" class="relative px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
        <div class="mx-auto max-w-7xl">
            <div class="lp-reveal mx-auto max-w-2xl text-center">
                ...
                <h2 class="mt-3 text-4xl font-extrabold tracking-tight text-slate-900 sm:text-5xl">{{ __('One card.') }} <span class="lp-grad">{{ __('Total control.') }}</span></h2>
            ...
            <div class="mt-14 grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
                {{-- Card stage --}}
                <div class="lp-reveal relative mx-auto w-full max-w-sm lg:mr-auto">
                    ...
                    <div class="relative" style="perspective:1600px">
                        <x-landing::wallet-card finish="aurora" balance="$12,480.55" number="4291" tilt />
                    </div>
                    {{-- Soft reflection --}}
                ...
                <div class="lp-reveal" data-d="1">
                    ...
                    <div class="grid gap-3 sm:grid-cols-2">
                        @foreach ($cardFeatures as $cf)
                            <div class="lp-glass lp-card-hover group rounded-2xl p-5">
                    ...
                    <div class="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
                        ...
                        <a href="{{ route('register') }}" class="lp-btn lp-btn-primary lp-btn-lg w-full sm:w-auto">{{ __('Get your card') }} <x-heroicon-o-arrow-right class="h-5 w-5" /></a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section id="wallet" class="relative px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
        <div class="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-2 lg:gap-16">
            ...
            <div class="lp-reveal">
                ...
                <h2 class="mt-3 text-4xl font-extrabold tracking-tight text-slate-900 sm:text-5xl">{{ __('Your whole portfolio,') }} <span class="lp-grad">{{ __('beautifully clear.') }}</span></h2>
                ...
                <a href="{{ route('register') }}" class="lp-btn lp-btn-primary lp-btn-lg mt-8 w-full sm:w-auto">{{ __('Open your wallet') }} <x-heroicon-o-arrow-right class="h-5 w-5" /></a>
            </div>

            {{-- Wallet app mock --}}
            <div class="lp-reveal relative mx-auto w-full max-w-md" data-d="1">
                ...
                <div class="lp-card overflow-hidden">
                    ...
                    <div class="relative overflow-hidden p-6 text-white" style="background:linear-gradient(135deg,#1e40af 0%,#2563eb 55%,#0ea5e9 100%)">
                        <span aria-hidden="true" class="lp-card-sheen pointer-events-none absolute inset-0"></span>
                        <div class="relative flex items-start justify-between">
                            <div>
                                <p class="text-[0.62rem] font-medium uppercase tracking-[0.18em] text-white/70">{{ __('Total balance') }}</p>
                                <p class="mt-1 text-3xl font-bold lp-tabular" data-count="17980" data-prefix="$" data-dec="2">$17,980.00</p>
                                <p class="mt-1 inline-flex items-center gap-1 text-sm font-medium lp-tabular text-white/90">
                            ...
                            <span class="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-2 py-0.5 text-[0.65rem] font-semibold backdrop-blur">
                                <span class="h-1.5 w-1.5 rounded-full lp-pulse" style="background:#fff"></span> Live
                            </span>
                        </div>
                            ...
                            <div class="flex items-center justify-between rounded-xl px-2 py-2 transition hover:bg-slate-50">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="leading-tight">
                                        ...
                                        <p class="text-xs text-slate-500 lp-tabular">{{ $a['amount'] }} {{ $a['sym'] }}</p>
                                    </div>
                                </div>
                                <div class="text-right leading-tight">
                                    <p class="text-sm font-semibold text-slate-900 lp-tabular">{{ $a['value'] }}</p>
                                    <p class="text-xs lp-tabular" style="color:{{ $a['up'] ? 'var(--up)' : 'var(--down)' }}">{{ $a['change'] }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="flex items-center justify-between border-t border-slate-100 px-6 py-4 text-xs">
                        <span class="inline-flex items-center gap-1.5 text-slate-400">
                            <span class="h-1.5 w-1.5 rounded-full lp-pulse" style="background:var(--up)"></span> {{ __('Updated just now') }}
                        </span>
    ...
    <section id="features" class="relative px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
        <div class="mx-auto max-w-7xl">
            ...
            <div class="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($why as $i => $w)
                    <div class="lp-reveal lp-glass lp-card-hover group relative overflow-hidden rounded-2xl p-6" data-d="{{ ($i % 4) + 1 }}">
                        {{-- Hover accent line --}}
    ...
    <section id="security" class="relative overflow-hidden px-4 py-24 sm:px-6 lg:px-8 lg:py-32">
        ...
        <div class="mx-auto max-w-7xl">
            ...
            <div class="lp-reveal mx-auto mt-12 grid max-w-3xl grid-cols-2 gap-4 sm:grid-cols-4" data-d="1">
                @foreach ($secStats as $st)
                    <div class="lp-glass rounded-2xl p-4 text-center">
                        <p class="text-2xl font-bold lp-tabular text-slate-900">{{ $st['value'] }}</p>
            ...
            <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($security as $i => $sec)
                    <div class="lp-reveal lp-glass lp-card-hover group rounded-2xl p-6" data-d="{{ ($i % 3) + 1 }}">
            ...
            {{-- Compliance chips --}}
            <div class="lp-reveal mt-10 flex flex-wrap items-center justify-center gap-2.5">
    ...
    <section id="faq" class="relative px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
        <div class="mx-auto max-w-3xl">
            <div class="lp-reveal text-center">
                ...
                <h2 class="mt-3 text-4xl font-extrabold tracking-tight text-slate-900 sm:text-5xl">{{ __('Questions,') }} <span class="lp-grad">{{ __('answered.') }}</span></h2>
            ...
            <div class="lp-reveal mt-10 space-y-3" x-data="{ open: null }">
                @foreach ($faqs as $i => $faq)
                    <div class="lp-glass overflow-hidden rounded-2xl">
    ...
    {{-- Alpine (lpTilt), scroll-reveal + counters, and the live price grid are all
         wired in the isolated landing bundle (resources/landing/js/*). --}}
