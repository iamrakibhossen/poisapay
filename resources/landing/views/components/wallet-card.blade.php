...
{{--
    ...
    Finish visuals live in the .lp-wrapper scoped stylesheet (home.blade.php).
--}}
<div
    @if ($tilt)
        x-data="lpTilt()" @mousemove="move($event)" @mouseleave="reset()"
        :style="`transform: perspective(1400px) rotateX(${rx}deg) rotateY(${ry}deg)`"
    @endif
    class="lp-card3d lp-card-{{ $finish }} group relative aspect-[1.586/1] w-full select-none overflow-hidden rounded-[1.5rem] p-6 text-white shadow-2xl"
    role="img"
...
>
    ...
    <div class="relative flex h-full flex-col justify-between">
        ...
        <div class="flex items-end justify-between">
            <div>
                <span aria-hidden="true" class="lp-card-chip block h-8 w-11 rounded-md"></span>
                <p class="mt-3 text-[0.62rem] uppercase tracking-[0.16em] text-white/55">{{ __('Balance') }}</p>
                <p class="text-lg font-bold lp-tabular tracking-tight">{{ $balance }}</p>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="mt-3 flex items-end justify-between">
                ...
                <div class="text-right leading-tight">
                    ...
                    <p class="text-[0.8rem] font-semibold lp-tabular tracking-wide">{{ $expiry }}</p>
                </div>
                @if ($network === 'visa')
