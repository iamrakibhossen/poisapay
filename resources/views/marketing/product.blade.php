    ...
    <section class="mx-auto max-w-7xl px-4 pt-14 sm:px-6 lg:px-8 sm:pt-20">
        <div class="grid items-center gap-12 lg:grid-cols-2">
            ...
            <div class="text-center lg:text-left">
                <span class="glass inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 text-xs font-semibold uppercase tracking-wider text-slate-600">
                ...
                <div class="mt-8 flex flex-col items-center gap-3 sm:flex-row lg:justify-start justify-center">
                    @if (! empty($product['cta']))
                        ...
                        <a href="{{ route($product['cta']['route']) }}" class="pp-btn pp-btn-primary pp-btn-lg">
                            {{ __($product['cta']['label']) }} <x-heroicon-o-arrow-right class="h-5 w-5" />
                        </a>
                        @if (! empty($product['pricing']))
                            <a href="#pricing" class="pp-btn pp-btn-ghost pp-btn-lg">{{ __('See plans') }}</a>
                        @else
                            @guest
                    ...
                    @else
                        @auth
                            <a href="{{ route('dashboard') }}" class="pp-btn pp-btn-primary pp-btn-lg">{{ __('Go to dashboard') }} <x-heroicon-o-arrow-right class="h-5 w-5" /></a>
                        @else
                            <a href="{{ route('register') }}" class="pp-btn pp-btn-primary pp-btn-lg">{{ __('Get started free') }} <x-heroicon-o-arrow-right class="h-5 w-5" /></a>
                            <a href="{{ route('login') }}" class="pp-btn pp-btn-ghost pp-btn-lg">{{ __('Log in') }}</a>
                        @endauth
                    @endif
                </div>
                ...
                @foreach ($product['pricing'] as $plan)
                    ...
                    <div @class([
                        ...
                        <a href="{{ ! empty($product['cta']) ? route($product['cta']['route']) : route('register') }}"
                            @class([
                                'pp-btn pp-btn-lg mt-8 w-full',
                                'pp-btn-primary' => $popular,
                                'pp-btn-ghost' => ! $popular,
                            ])>{{ __('Start selling') }}</a>
                    </div>
                @endforeach
            </div>
