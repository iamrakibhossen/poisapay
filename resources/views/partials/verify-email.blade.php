@if (auth()->check()
    ...
    <div x-data="{ show: true }" x-show="show" x-cloak
        ...
        <p class="min-w-0 flex-1 text-sm text-amber-800">
            <span class="font-medium">{{ __('app.verify_email') }}.</span>
            Please verify <span class="font-medium">{{ auth()->user()->email }}</span> to unlock all features.
        </p>
        ...
        <button type="button" @click="show = false"
                class="rounded-lg p-1.5 text-amber-500 hover:bg-amber-100" aria-label="Dismiss">
