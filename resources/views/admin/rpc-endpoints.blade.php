<x-layouts.admin :title="'RPC Endpoints'">
    ...
    <div x-data="{
        ...
        <x-ui.page-header title="RPC Endpoints" subtitle="JSON-RPC nodes the indexer and broadcaster failover across, ordered by priority per chain (Phase 3).">
            <x-slot:actions>
                <x-ui.button x-on:click="create()" icon="plus" size="sm">Add endpoint</x-ui.button>
            </x-slot:actions>
        </x-ui.page-header>

        <x-ui.table :headers="['Chain', 'Name', 'URL', 'Priority', 'Weight', 'Status', 'Last block', 'Latency', 'Checked', 'Active', '']">
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-2">
                            ...
                            <x-ui.copy-text :text="$e->url" label="Copy URL" />
                        </div>
                    </td>
                    ...
                    <td class="px-4 py-3">
                        <form method="POST" action="{{ route('admin.rpc-endpoints.toggle', $e->id) }}">
                            ...
                            <button type="submit" class="inline-flex">
                                <x-ui.badge :color="$e->is_active ? 'success' : 'gray'" dot>{{ $e->is_active ? 'Active' : 'Disabled' }}</x-ui.badge>
                            </button>
                        </form>
                    </td>
                    <td class="px-4 py-3 text-right">
                        <div class="flex justify-end gap-2">
                            <x-ui.button variant="secondary" size="sm" icon="pencil-square"
                                x-on:click="edit({{ Illuminate\Support\Js::from(['id' => $e->id, 'chain_id' => $e->chain_id, 'name' => $e->name, 'url' => $e->url, 'priority' => (int) $e->priority, 'weight' => (int) $e->weight, 'is_active' => (bool) $e->is_active]) }})">Edit</x-ui.button>
            ...
            @empty
                <tr><td colspan="11"><x-ui.empty-state icon="server-stack" title="No RPC endpoints" description="Add a JSON-RPC node so the indexer can sync a chain." /></td></tr>
            @endforelse
        </x-ui.table>
        ...
        <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-lg pp-card p-6">
                ...
                <form method="POST" action="{{ route('admin.rpc-endpoints.save') }}" class="space-y-4">
                    ...
                    <x-ui.input label="Name" name="name" x-model="form.name" placeholder="Ankr primary" :error="$errors->first('name')" />
                    <x-ui.input label="URL" name="url" x-model="form.url" placeholder="https://rpc.example.com" :error="$errors->first('url')" />
                    <div class="grid gap-4 sm:grid-cols-2">
                        <x-ui.input type="number" label="Priority (1 = highest)" name="priority" x-model="form.priority" min="1" max="99" :error="$errors->first('priority')" />
                        <x-ui.input type="number" label="Weight (load balance)" name="weight" x-model="form.weight" min="1" max="100" :error="$errors->first('weight')" />
                    </div>
