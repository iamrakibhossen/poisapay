<x-layouts.admin :title="'Ledger'">
    <div class="space-y-6" x-data="{ reversingId: null }">
        <x-ui.page-header title="Ledger" subtitle="Double-entry journal. Every money movement posts a balanced set of lines." />

        <form method="GET" action="{{ route('admin.ledger') }}" class="flex justify-end">
            <x-ui.input name="search" :value="$search" icon="magnifying-glass" placeholder="Search type or idempotency key…" class="w-full sm:w-80" />
        </form>

        <x-ui.table :headers="['Entry', 'Type', 'Status', 'Memo', 'Lines', 'Created', '']">
            @forelse ($entries as $entry)
                ...
                <tr class="cursor-pointer hover:bg-neutral-50">
                    ...
                    <td class="px-4 py-3">
                        ...
                        @if ($entry->reverses_entry_id)
                            <x-ui.badge color="gray">Reversal</x-ui.badge>
                        @endif
                    </td>
                    ...
                    <td class="px-4 py-3 text-right">
                        @if ($canReverse && $entry->status !== \App\Enums\EntryStatus::Reversed && $entry->reverses_entry_id === null)
                            <x-ui.button type="button" x-on:click="reversingId = '{{ $entry->id }}'" variant="danger" size="sm" icon="arrow-uturn-left">Reverse</x-ui.button>
                        @endif
                    </td>
                </tr>
                            ...
                            <div class="overflow-hidden rounded-xl border border-neutral-200">
                                <table class="min-w-full divide-y divide-neutral-200">
                                    <thead class="bg-neutral-100/70">
                                        <tr>
                                            <th class="px-4 py-2 text-left text-[11px] font-semibold uppercase tracking-wider text-neutral-500">Side</th>
                                            <th class="px-4 py-2 text-left text-[11px] font-semibold uppercase tracking-wider text-neutral-500">Account</th>
                                            <th class="px-4 py-2 text-right text-[11px] font-semibold uppercase tracking-wider text-neutral-500">Amount</th>
                                        </tr>
                                    </thead>
                ...
                @endif
            @empty
                <tr><td colspan="7"><x-ui.empty-state icon="book-open" title="No journal entries" description="Nothing posted to the ledger yet." /></td></tr>
            @endforelse
        </x-ui.table>
                    ...
                    <div x-show="reversingId === '{{ $entry->id }}'" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <div class="relative w-full max-w-md pp-card p-6" role="dialog" aria-modal="true">
                            <div class="mb-4 flex items-center justify-between">
                                <h3 class="text-lg font-semibold text-neutral-900">Reverse entry</h3>
                            ...
                            <form method="POST" action="{{ route('admin.ledger.reverse', $entry->id) }}" class="space-y-4">
                                ...
                                <p class="text-sm text-neutral-600">
                                    This posts a new balanced reversing entry and marks the original as
                                    <span class="font-semibold">reversed</span>. This cannot be undone.
                                </p>
                                <x-ui.input label="Reason (optional)" name="reason" placeholder="e.g. Duplicate posting" class="w-full" />
                                <div class="flex justify-end gap-2">
                                    <x-ui.button type="button" variant="ghost" size="sm" x-on:click="reversingId = null">Cancel</x-ui.button>
                                    <x-ui.button type="submit" variant="danger" size="sm" icon="arrow-uturn-left">Reverse entry</x-ui.button>
                                </div>
                            </form>
                        </div>
                    </div>
