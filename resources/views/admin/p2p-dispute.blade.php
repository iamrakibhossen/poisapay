<x-layouts.admin :title="'Dispute case'">
    @php
        $order = $dispute->order;
        $canManage = auth('admin')->user()?->can('manage-p2p') || auth('admin')->user()?->hasRole('super-admin');
    @endphp
    <div class="space-y-6">
        <x-ui.page-header :title="'Dispute — '.($order?->ref ?? '')" subtitle="Full case history: timeline, chat transcript and evidence. Rule for the buyer to force-release, or the seller to refund.">
            <x-slot:actions>
                <a href="{{ route('admin.p2p-disputes') }}"><x-ui.button variant="secondary" icon="arrow-left">All disputes</x-ui.button></a>
            </x-slot:actions>
        </x-ui.page-header>
        ...
        <div class="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
            ...
            <div class="space-y-4">
                <x-ui.card>
                    <div class="flex items-center gap-2">
                        ...
                        @if ($dispute->assignedAdmin)<span class="text-xs text-neutral-500">assigned to {{ $dispute->assignedAdmin->name }}</span>@endif
                    </div>
                    ...
                    <p class="mt-2 text-xs text-neutral-500">Opened by {{ $dispute->opened_by_role }} · {{ $dispute->created_at?->diffForHumans() }}</p>

                    <dl class="mt-4 divide-y divide-neutral-100 text-sm">
                        <div class="flex justify-between py-2"><dt class="text-neutral-500">Amount</dt><dd class="tabular text-neutral-900">{{ $order?->cryptoMoney()->format() }}</dd></div>
                        <div class="flex justify-between py-2"><dt class="text-neutral-500">Fiat</dt><dd class="tabular text-neutral-700">{{ number_format((float) ($order?->fiat_amount ?? 0), 2) }} {{ $order?->fiat_currency }}</dd></div>
                        <div class="flex justify-between py-2"><dt class="text-neutral-500">Buyer</dt><dd class="text-neutral-900">{{ $order?->buyer?->name }}</dd></div>
                        <div class="flex justify-between py-2"><dt class="text-neutral-500">Seller</dt><dd class="text-neutral-900">{{ $order?->seller?->name }}</dd></div>
                    </dl>
                    ...
                    @if ($canManage && $dispute->status->isOpen())
                        @if ($dispute->status->value === 'open')
                            <form method="POST" action="{{ route('admin.p2p-disputes.assign', $dispute) }}" class="mt-4">
                                @csrf
                                <x-ui.button type="submit" variant="secondary" icon="hand-raised" class="w-full">Take case (mark under review)</x-ui.button>
                            </form>
                        @endif
                        <form method="POST" action="{{ route('admin.p2p-disputes.resolve', $dispute) }}" class="mt-4 space-y-3 border-t border-neutral-100 pt-4">
                            @csrf
                            <div>
                                <label class="pp-label">Resolution note</label>
                ...
                <x-ui.card>
                    <h3 class="text-sm font-semibold text-neutral-900">Evidence</h3>
                    <div class="mt-3 space-y-2">
                        @forelse ($dispute->evidence as $ev)
                            <div class="flex items-center justify-between rounded-lg border border-neutral-200 px-3 py-2 text-sm">
                                ...
                                <a href="{{ route('admin.p2p-disputes.evidence', $ev) }}" target="_blank" class="ml-3 shrink-0 text-brand-600 hover:underline">Download</a>
                            </div>
                        @empty
                            <p class="text-sm text-neutral-400">No evidence uploaded.</p>
                        @endforelse
                    </div>
                </x-ui.card>
            </div>
            ...
            <div class="space-y-4">
                <x-ui.card>
                    <h3 class="text-sm font-semibold text-neutral-900">Order timeline</h3>
                ...
                <x-ui.card>
                    <h3 class="text-sm font-semibold text-neutral-900">Chat transcript</h3>
                    <div class="mt-3 max-h-96 space-y-2 overflow-y-auto">
                        @forelse ($order?->messages->sortBy('created_at') ?? [] as $m)
                            <div class="text-sm">
                                <span class="font-medium text-neutral-600">{{ $m->sender_type === 'system' ? 'System' : ($m->sender_id === $order->buyer_id ? 'Buyer' : 'Seller') }}:</span>
                                <span class="text-neutral-800">{{ $m->body ?: '[attachment]' }}</span>
                                <span class="text-xs text-neutral-400">· {{ $m->created_at?->format('d M H:i') }}</span>
                            </div>
                        @empty
                            <p class="text-sm text-neutral-400">No messages.</p>
                        @endforelse
                    </div>
                </x-ui.card>
            </div>
