<x-layouts.admin :title="__('P2P Disputes')">
    @php $canManage = auth('admin')->user()?->can('manage-p2p') || auth('admin')->user()?->hasRole('super-admin'); @endphp
    <div class="space-y-6">
        <x-ui.page-header :title="__('P2P Disputes')" :subtitle="__('Adjudicate contested trades. Ruling for the buyer force-releases escrow; ruling for the seller refunds it.')" />
        ...
        <div class="space-y-4">
                ...
                <x-ui.card>
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        <div class="min-w-0">
                            ...
                            <p class="mt-2 text-xs text-neutral-500">
                                {{ __('Opened by') }} {{ $dispute->opened_by_role }} ·
                                {{ $order?->cryptoMoney()->format() }} {{ __('for') }} {{ number_format((float) ($order?->fiat_amount ?? 0), 2) }} {{ $order?->fiat_currency }} ·
                                {{ __('buyer') }} {{ $order?->buyer?->name }} / {{ __('seller') }} {{ $order?->seller?->name }}
                            </p>
                        </div>
                        <a href="{{ route('admin.p2p-disputes.show', $dispute) }}"><x-ui.button size="sm" variant="secondary" icon="eye">{{ __('Open case') }}</x-ui.button></a>
                    </div>
                    ...
                    @if ($canManage && $dispute->status->isOpen())
                        <form method="POST" action="{{ route('admin.p2p-disputes.resolve', $dispute) }}" class="mt-4 flex flex-col gap-3 border-t border-gray-200 pt-4 sm:flex-row sm:items-end">
                            @csrf
                            <div class="flex-1">
                                <label class="pp-label">{{ __('Resolution note (optional)') }}</label>
            ...
            @empty
                <x-ui.card><x-ui.empty-state icon="scale" :title="__('No disputes')" :description="__('There are no P2P disputes right now.')" /></x-ui.card>
            @endforelse
        </div>
