<x-layouts.admin :title="__('Administrators')">
    ...
    <div x-data="{
        ...
        <x-ui.page-header :title="__('Administrators')" :subtitle="__('Operator accounts and the roles assigned to them.')">
            <x-slot:actions>
                <x-ui.button x-on:click="create()" icon="plus" size="sm">{{ __('Add administrator') }}</x-ui.button>
            </x-slot:actions>
        </x-ui.page-header>

        <x-ui.table :headers="[__('Administrator'), __('Roles'), __('Status'), __('Last login'), __('Created'), '']">
            @forelse ($admins as $admin)
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td class="px-4 py-3">
                        @if ($admin->id === $currentId)
                            <span class="inline-flex cursor-not-allowed opacity-60">
                                <x-ui.badge :color="$admin->is_active ? 'success' : 'gray'" dot>{{ $admin->is_active ? __('Active') : __('Disabled') }}</x-ui.badge>
                            </span>
                        @else
                    ...
                    <td class="px-4 py-3 text-sm text-neutral-600">{{ $admin->last_login_at?->diffForHumans() ?? __('Never') }}</td>
                    ...
                    <td class="px-4 py-3">
                        <div class="flex justify-end gap-2">
                            <x-ui.button variant="secondary" size="sm" icon="pencil-square"
                                x-on:click="edit({{ Illuminate\Support\Js::from(['id' => (string) $admin->id, 'name' => $admin->name, 'email' => $admin->email, 'username' => $admin->username, 'is_active' => (bool) $admin->is_active, 'roles' => $admin->roles->pluck('name')->all()]) }})">{{ __('Edit') }}</x-ui.button>
                            @if ($admin->id !== $currentId)
            ...
            @empty
                <tr><td colspan="6"><x-ui.empty-state icon="users" :title="__('No administrators')" :description="__('Add an operator account to grant admin access.')" /></td></tr>
            @endforelse
        </x-ui.table>
            ...
            <div class="relative flex max-h-[90vh] w-full max-w-2xl flex-col pp-card p-6" role="dialog" aria-modal="true">
                ...
                <form method="POST" action="{{ route('admin.administrators.save') }}" class="flex min-h-0 flex-1 flex-col space-y-4 overflow-y-auto pr-1">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <x-ui.input :label="__('Name')" name="name" x-model="form.name" :placeholder="__('Jane Operator')" :error="$errors->first('name')" />
                        <x-ui.input :label="__('Email')" type="email" name="email" x-model="form.email" :placeholder="__('jane@poisapay.com')" :error="$errors->first('email')" />
                    </div>
                    <div class="grid gap-4 sm:grid-cols-2">
                        <x-ui.input :label="__('Username (optional)')" name="username" x-model="form.username" :placeholder="__('jane')" :error="$errors->first('username')" />
                        <x-ui.input :label="__('Password')" type="password" name="password" x-model="form.password" autocomplete="new-password" :hint="__('Minimum 8 characters. Leave blank when editing to keep current.')" :error="$errors->first('password')" />
                    </div>

                    <div>
                        <label class="pp-label">{{ __('Roles') }}</label>
                        <div class="grid gap-2 rounded-lg border border-gray-200 p-4 sm:grid-cols-2">
                            ...
                            @empty
                                <p class="text-sm text-neutral-400">{{ __('No roles defined yet.') }}</p>
                            @endforelse
                        </div>
                    </div>

                    <label class="flex items-center gap-2 text-sm text-neutral-700">
                        <input type="checkbox" name="is_active" value="1" x-model="form.is_active" class="rounded border-neutral-300 text-brand-500 focus:ring-brand-500"> {{ __('Active') }}
                    </label>
