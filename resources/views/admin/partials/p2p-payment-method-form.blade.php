    ...
    <div class="w-full max-w-2xl rounded-2xl bg-white shadow-xl" @click.outside="open = false">
        ...
        <form method="POST" :action="action" class="space-y-5 px-6 py-5">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <label class="pp-label">{{ __('Name') }}</label>
                    <input type="text" name="name" x-model="form.name" class="pp-input" required placeholder="{{ __('bKash') }}">
                    @error('name')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="pp-label">{{ __('Key') }}</label>
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                <div>
                    <label class="pp-label">{{ __('Type') }}</label>
            ...
            <div class="border-t border-gray-100 pt-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-semibold text-gray-900">{{ __('Account fields') }}</p>
                        <p class="text-xs text-gray-500">{{ __('What a user enters when saving an account on this method.') }}</p>
                    </div>
                    <x-ui.button type="button" size="sm" variant="secondary" icon="plus" x-on:click="addField()">{{ __('Add field') }}</x-ui.button>
                </div>

                <div class="mt-3 space-y-2">
                    <template x-for="(f, i) in fields" :key="i">
                        <div class="flex items-center gap-2">
                            <input type="text" :name="`fields[${i}][key]`" x-model="f.key" placeholder="{{ __('key (e.g. bank_name)') }}" class="pp-input font-mono flex-1">
                            <input type="text" :name="`fields[${i}][label]`" x-model="f.label" placeholder="{{ __('Label') }}" class="pp-input flex-1">
                            <label class="inline-flex shrink-0 items-center gap-1.5 text-xs text-gray-600" title="{{ __('Required') }}">
                    ...
                    <p x-show="fields.length === 0" class="text-xs text-gray-400">{{ __('No fields — add at least one.') }}</p>
                </div>
            </div>

            <div class="flex justify-end gap-3 border-t border-gray-100 pt-4">
                <x-ui.button type="button" variant="secondary" x-on:click="open = false">{{ __('Cancel') }}</x-ui.button>
                <x-ui.button type="submit" x-text="editingId ? 'Save method' : 'Create method'">{{ __('Save') }}</x-ui.button>
            </div>
        </form>
    </div>
</div>
