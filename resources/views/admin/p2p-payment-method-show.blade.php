<x-layouts.admin :title="'Payment method · '.$method->name">
    <div x-data="{
        ...
        <a href="{{ route('admin.p2p-payment-methods') }}" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800">
            <x-heroicon-o-arrow-left class="h-4 w-4" /> P2P Payment Methods
        </a>

        <x-ui.page-header :title="$method->name" :subtitle="'Rail configuration and the accounts users have saved on it.'">
            <x-slot:actions>
                <x-ui.button x-on:click="open = true" icon="pencil-square" size="sm" variant="secondary">Edit</x-ui.button>
                <form method="POST" action="{{ route('admin.p2p-payment-methods.delete', $method) }}">
                    @csrf @method('DELETE')
                    <x-ui.button type="submit" size="sm" variant="ghost" icon="trash">Delete</x-ui.button>
                </form>
            </x-slot:actions>
        </x-ui.page-header>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            {{-- Configuration --}}
            <x-ui.card title="Configuration" class="lg:col-span-2">
                <dl class="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3">
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wide text-gray-400">Key</dt>
                    ...
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wide text-gray-400">Country</dt>
                    ...
                    <div>
                        ...
                        <dd class="mt-1"><x-ui.badge :color="$method->is_active ? 'success' : 'gray'" dot>{{ $method->is_active ? 'Active' : 'Off' }}</x-ui.badge></dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wide text-gray-400">Sort</dt>
                        <dd class="mt-1 text-sm text-gray-900 tabular">{{ (int) $method->sort }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium uppercase tracking-wide text-gray-400">Saved accounts</dt>
            ...
            <x-ui.card title="Account fields" subtitle="What a user enters when saving an account.">
                    ...
                    <ul class="space-y-2">
                        @foreach ($fields as $f)
                            <li class="flex items-center justify-between gap-3 rounded-lg border border-gray-100 bg-gray-50/60 px-3 py-2">
                                ...
                                @if (! empty($f['required']))
                                    <x-ui.badge color="warning">Required</x-ui.badge>
                                @else
        ...
        <div>
            <div class="mb-3 flex items-center gap-2">
                <h2 class="text-sm font-semibold text-gray-900">User accounts</h2>
            ...
            @if ($accounts->count())
                <x-ui.table :headers="['User', 'Label', 'Status', 'Added']">
                    @foreach ($accounts as $acct)
                        <tr class="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                            <td class="px-4 py-3">
                                <p class="font-medium text-gray-900">{{ $acct->user?->name ?? 'Unknown' }}</p>
                                <p class="text-xs text-gray-400">{{ $acct->user?->email }}</p>
                            </td>
                            <td class="px-4 py-3 text-gray-700">{{ $acct->label ?: '—' }}</td>
                            <td class="px-4 py-3"><x-ui.badge :color="$acct->is_active ? 'success' : 'gray'" dot>{{ $acct->is_active ? 'Active' : 'Off' }}</x-ui.badge></td>
                ...
                <p class="mt-3 flex items-center gap-1.5 text-xs text-gray-400">
                    ...
                    Account details are encrypted and never shown here.
                </p>
            ...
            @else
                <x-ui.card>
                    <x-ui.empty-state icon="user-group" title="No accounts yet" description="No users have saved a payout account on this rail." />
                </x-ui.card>
            @endif
        </div>
