<x-admin.form-layout title="Transfer" description="Peer-to-peer transfers between PoisaPay users." class="!my-0">
    <form class="space-y-5" method="POST" action="{{ route('admin.settings.update', 'transfer') }}">
        @csrf
        @method('PUT')

        <x-admin.input.group id="transfer_enabled" label="Enable Transfers" class="w-full" hints="Allow users to send funds to each other.">
