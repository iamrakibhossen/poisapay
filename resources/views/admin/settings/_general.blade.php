<x-admin.form-layout title="General Settings" description="Core identity and operational state of the platform." class="!my-0">
    <form class="space-y-5" method="POST" action="{{ route('admin.settings.update', 'general') }}">
        @csrf
        @method('PUT')

        <x-admin.input.group id="site_name" label="Site Name" required class="w-full">
            <x-admin.input name="site_name" :value="old('site_name', getSetting('site_name', 'PoisaPay'))" required />
        </x-admin.input.group>

        <x-admin.input.group id="site_slogan" label="Slogan" class="w-full">
            <x-admin.input name="site_slogan" :value="old('site_slogan', getSetting('site_slogan'))" />
        </x-admin.input.group>

        <x-admin.input.group id="base_currency" label="Base Currency" required class="w-full"
            hints="The currency balances, reports and limits are denominated in.">
        ...
        <x-admin.input.group id="support_email" label="Support Email" required class="w-full">
            <x-admin.input type="email" name="support_email" :value="old('support_email', getSetting('support_email'))" required />
        </x-admin.input.group>

        <x-admin.input.group id="maintenance_mode" label="Maintenance Mode" class="w-full"
            hints="Takes the platform offline for non-admins.">
