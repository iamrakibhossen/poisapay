<x-admin.form-layout title="Branding" description="Colours and logos applied across the app." class="!my-0">
    <form class="space-y-5" method="POST" action="{{ route('admin.settings.update', 'branding') }}">
        @csrf
        @method('PUT')

        <x-admin.input.group id="primary_color" label="Primary Colour" required class="w-full">
        ...
        <x-admin.input.group id="secondary_color" label="Secondary Colour" required class="w-full">
            <div class="flex items-center gap-3">
        ...
        <x-admin.input.group id="site_logo" label="Logo URL" class="w-full">
            <x-admin.input name="site_logo" placeholder="https://…" :value="old('site_logo', getSetting('site_logo'))" />
        </x-admin.input.group>

        <x-admin.input.group id="site_favicon" label="Favicon URL" class="w-full">
