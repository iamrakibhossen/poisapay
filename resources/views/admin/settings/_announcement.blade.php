<x-admin.form-layout :title="__('Announcement')" :description="__('A banner shown across the top of the app.')" class="!my-0">
    <form class="space-y-5" method="POST" action="{{ route('admin.settings.update', 'announcement') }}">
        @csrf
        @method('PUT')

        <x-admin.input.group id="header_announcement_enabled" :label="__('Show Header Announcement')" class="w-full">
        ...
        <x-admin.input.group id="header_announcement_text" :label="__('Announcement Text')" class="w-full">
            <x-admin.input name="header_announcement_text" :value="old('header_announcement_text', getSetting('header_announcement_text'))" />
        </x-admin.input.group>

        <x-admin.input.group id="header_announcement_link" :label="__('Link (optional)')" class="w-full">
