<x-admin.form-layout :title="__('Cards')" :description="__('Defaults and fees for the card program.')" class="!my-0">
    <form class="space-y-5" method="POST" action="{{ route('admin.settings.update', 'cards') }}">
        @csrf
        @method('PUT')

        <x-admin.input.group id="cards_enabled" :label="__('Enable Cards')" class="w-full">
        ...
        <x-admin.input.group id="card_default_daily_limit" :label="__('Default Daily Limit')" required class="w-full" :hints="__('Settlement-currency minor units (e.g. 500000 = $5,000.00).')">
        ...
        <x-admin.input.group id="card_default_per_tx_limit" :label="__('Default Per-transaction Limit')" required class="w-full" :hints="__('Settlement-currency minor units.')">
        ...
        <x-admin.input.group id="card_dispute_window_days" :label="__('Dispute Window (days)')" required class="w-full">
        ...
        <x-admin.input.group id="card_allow_physical" :label="__('Allow Physical Cards')" class="w-full">
            <x-admin.input.boolean name="card_allow_physical" :value="old('card_allow_physical', getSetting('card_allow_physical', true))" />
        </x-admin.input.group>

        <x-admin.input.group id="card_reveal_enabled" :label="__('Allow Card Detail Reveal')" class="w-full" :hints="__('Let users reveal card details via the PCI-compliant iframe.')">
