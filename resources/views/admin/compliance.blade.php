<x-layouts.admin :title="__('Compliance')">
    @php
    ...
    >
        <x-ui.page-header :title="__('Compliance')" :subtitle="__('AML alerts, cases and sanctions screening.')" />

        {{-- Stat cards --}}
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <x-ui.stat-card :label="__('Open alerts')" :value="number_format($stats['openAlerts'])" icon="bell-alert" accent="amber" />
            <x-ui.stat-card :label="__('Escalated alerts')" :value="number_format($stats['escalatedAlerts'])" icon="exclamation-triangle" accent="rose" />
            <x-ui.stat-card :label="__('Open cases')" :value="number_format($stats['openCases'])" icon="folder-open" accent="brand" />
            <x-ui.stat-card :label="__('SARs filed')" :value="number_format($stats['sarsFiled'])" icon="document-text" accent="emerald" />
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-5">
            ...
            <nav class="flex gap-1 overflow-x-auto rounded-xl border border-gray-200 bg-white p-2 lg:sticky lg:top-6 lg:col-span-1 lg:flex-col lg:self-start lg:overflow-visible">
                @foreach (['alerts' => ['label' => __('Alerts'), 'icon' => 'bell-alert'], 'cases' => ['label' => __('Cases'), 'icon' => 'folder-open'], 'screening' => ['label' => __('Screening'), 'icon' => 'shield-check']] as $key => $meta)
                    <a
            ...
            <div class="space-y-6 lg:col-span-4">
                ...
                <form method="GET" action="{{ route('admin.compliance') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @if ($tab === 'alerts')
                            <x-ui.select name="alertStatus" class="w-auto" x-on:change="$el.form.submit()">
                                <option value="all" @selected($alertStatus === 'all')>{{ __('All statuses') }}</option>
                                <option value="open" @selected($alertStatus === 'open')>{{ __('Open') }}</option>
                                <option value="escalated" @selected($alertStatus === 'escalated')>{{ __('Escalated') }}</option>
                                <option value="cleared" @selected($alertStatus === 'cleared')>{{ __('Cleared') }}</option>
                            </x-ui.select>
                            <x-ui.select name="severity" class="w-auto" x-on:change="$el.form.submit()">
                                <option value="all" @selected($severity === 'all')>{{ __('All severities') }}</option>
                        ...
                        @elseif ($tab === 'cases')
                            <x-ui.select name="caseStatus" class="w-auto" x-on:change="$el.form.submit()">
                                <option value="all" @selected($caseStatus === 'all')>{{ __('All statuses') }}</option>
                                <option value="open" @selected($caseStatus === 'open')>{{ __('Open') }}</option>
                                <option value="investigating" @selected($caseStatus === 'investigating')>{{ __('Investigating') }}</option>
                                <option value="closed" @selected($caseStatus === 'closed')>{{ __('Closed') }}</option>
                            </x-ui.select>
                            <x-ui.select name="riskLevel" class="w-auto" x-on:change="$el.form.submit()">
                                <option value="all" @selected($riskLevel === 'all')>{{ __('All risk levels') }}</option>
                                @foreach ($severities as $level)
                    ...
                    <x-ui.input name="search" :value="$search" icon="magnifying-glass" :placeholder="__('Search user name or email…')" class="w-full sm:w-72" />
                </form>

                {{-- ALERTS TAB --}}
                @if ($tab === 'alerts')
                    <x-ui.table :headers="[__('User'), __('Type'), __('Severity'), __('Context'), __('Score'), __('Status'), __('Created'), '']">
                                ...
                                <td class="px-3 py-3">
                                    <div class="flex items-center justify-end gap-1.5">
                                        @if ($alert->status !== \App\Enums\AlertStatus::Cleared)
                                            <x-ui.button type="button" x-on:click="clearingId = '{{ $alert->id }}'" variant="success" size="sm" icon="check">{{ __('Clear') }}</x-ui.button>
                                            @if ($alert->status !== \App\Enums\AlertStatus::Escalated)
                                                <form method="POST" action="{{ route('admin.compliance.alert.escalate', $alert->id) }}" onsubmit="return confirm('Escalate this alert?')">
                                                    @csrf
                                                    <x-ui.button type="submit" variant="danger" size="sm" icon="arrow-trending-up">{{ __('Escalate') }}</x-ui.button>
                                                </form>
                                            @endif
                                        @endif
                                        <x-ui.button type="button" x-on:click="viewingAlert = '{{ $alert->id }}'" variant="secondary" size="sm" icon="eye">{{ __('View') }}</x-ui.button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="8"><x-ui.empty-state icon="bell-alert" :title="__('No alerts')" :description="__('No AML alerts match your filters.')" /></td></tr>
                        @endforelse
                    </x-ui.table>
                ...
                {{-- CASES TAB --}}
                @if ($tab === 'cases')
                    <x-ui.table :headers="[__('User'), __('Reason'), __('Risk'), __('Status'), __('Alerts'), __('SAR'), __('Assignee'), __('Opened'), '']">
                        @forelse ($cases as $case)
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td class="px-3 py-3">
                                    @if ($case->sar_filed)
                                        <x-ui.badge color="indigo" icon="document-text">{{ __('Filed') }}</x-ui.badge>
                                    @else
                                    ...
                                    <div class="flex items-center justify-end gap-1.5">
                                        @if ($case->status !== \App\Enums\CaseStatus::Closed)
                                            <form method="POST" action="{{ route('admin.compliance.alert.assign', $case->id) }}" onsubmit="return confirm('Assign this case to yourself?')">
                                                @csrf
                                                <x-ui.button type="submit" variant="ghost" size="sm" icon="user-plus">{{ __('Assign to me') }}</x-ui.button>
                                            </form>
                                            @if ($canSar && ! $case->sar_filed)
                                                <x-ui.button type="button" x-on:click="sarId = '{{ $case->id }}'" variant="secondary" size="sm" icon="document-text">{{ __('File SAR') }}</x-ui.button>
                                            @endif
                                            <x-ui.button type="button" x-on:click="closingId = '{{ $case->id }}'" variant="danger" size="sm" icon="lock-closed">{{ __('Close') }}</x-ui.button>
                                        @endif
                                        <x-ui.button type="button" x-on:click="viewingCase = '{{ $case->id }}'" variant="secondary" size="sm" icon="eye">{{ __('View') }}</x-ui.button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="9"><x-ui.empty-state icon="folder-open" :title="__('No cases')" :description="__('No compliance cases match your filters.')" /></td></tr>
                        @endforelse
                    </x-ui.table>
                ...
                {{-- SCREENING TAB --}}
                @if ($tab === 'screening')
                    <x-ui.table :headers="[__('User'), __('Context'), __('Result'), __('Score'), __('Provider'), __('Matches'), __('Created')]">
                        ...
                        @empty
                            <tr><td colspan="7"><x-ui.empty-state icon="shield-check" :title="__('No screenings')" :description="__('No sanctions screening results found.')" /></td></tr>
                        @endforelse
                    </x-ui.table>
                    ...
                    <div x-show="clearingId === '{{ $alert->id }}'" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <div class="relative w-full max-w-md pp-card p-6" role="dialog" aria-modal="true">
                            <div class="mb-4 flex items-start justify-between">
                                <h3 class="text-lg font-semibold text-neutral-900">{{ __('Clear alert') }}</h3>
                            ...
                            <p class="mb-4 text-sm text-neutral-500">{{ __('Clearing marks this alert as resolved. Add an optional note explaining the decision.') }}</p>
                            <form method="POST" action="{{ route('admin.compliance.alert.clear', $alert->id) }}" class="space-y-4">
                                @csrf
                                <x-ui.textarea :label="__('Resolution note (optional)')" name="clearNote" :rows="3" :placeholder="__('e.g. Reviewed, false positive')" :error="$errors->first('clearNote')" />
                                <div class="flex justify-end gap-2">
                                    <x-ui.button type="button" variant="secondary" x-on:click="clearingId = null">{{ __('Cancel') }}</x-ui.button>
                                    <x-ui.button type="submit" variant="success" icon="check">{{ __('Clear alert') }}</x-ui.button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                @if ($case->status !== \App\Enums\CaseStatus::Closed)
                    @if ($canSar && ! $case->sar_filed)
                        <div x-show="sarId === '{{ $case->id }}'" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                            ...
                            <div class="relative w-full max-w-md pp-card p-6" role="dialog" aria-modal="true">
                                <div class="mb-4 flex items-start justify-between">
                                    <h3 class="text-lg font-semibold text-neutral-900">{{ __('File SAR') }}</h3>
                                ...
                                <p class="mb-4 text-sm text-neutral-500">{{ __('Record a Suspicious Activity Report reference against this case.') }}</p>
                                <form method="POST" action="{{ route('admin.compliance.case.sar', $case->id) }}" class="space-y-4">
                                    @csrf
                                    <x-ui.input :label="__('Reference')" name="sarReference" type="text" :placeholder="__('e.g. SAR-2026-0042')" :error="$errors->first('sarReference')" />
                                    <x-ui.textarea :label="__('Summary (optional)')" name="sarSummary" :rows="3" :placeholder="__('Brief narrative for the filing')" :error="$errors->first('sarSummary')" />
                                    <div class="flex justify-end gap-2">
                                        <x-ui.button type="button" variant="secondary" x-on:click="sarId = null">{{ __('Cancel') }}</x-ui.button>
                                        <x-ui.button type="submit" variant="primary" icon="document-text">{{ __('File SAR') }}</x-ui.button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    ...
                    <div x-show="closingId === '{{ $case->id }}'" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <div class="relative w-full max-w-md pp-card p-6" role="dialog" aria-modal="true">
                            <div class="mb-4 flex items-start justify-between">
                                <h3 class="text-lg font-semibold text-neutral-900">{{ __('Close case') }}</h3>
                            ...
                            <p class="mb-4 text-sm text-neutral-500">{{ __('Closing clears any still-open alerts under the same decision. Provide a resolution.') }}</p>
                            <form method="POST" action="{{ route('admin.compliance.case.close', $case->id) }}" class="space-y-4">
                                @csrf
                                <x-ui.textarea :label="__('Resolution')" name="closeResolution" :rows="3" :placeholder="__('e.g. No suspicious activity confirmed')" :error="$errors->first('closeResolution')" />
                                <div class="flex justify-end gap-2">
                                    <x-ui.button type="button" variant="secondary" x-on:click="closingId = null">{{ __('Cancel') }}</x-ui.button>
                                    <x-ui.button type="submit" variant="danger" icon="lock-closed">{{ __('Close case') }}</x-ui.button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                <div x-show="viewingAlert === '{{ $alert->id }}'" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative flex max-h-[85vh] w-full max-w-lg flex-col pp-card p-6" role="dialog" aria-modal="true">
                        ...
                        <div class="mb-4 flex flex-wrap items-center gap-2">
                            ...
                            <span class="tabular text-xs text-neutral-500">{{ __('Score') }} {{ $alert->score }}</span>
                        ...
                        <div class="min-h-0 flex-1 overflow-y-auto">
                            <dl class="mb-4 grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                                <div>
                                    <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Context') }}</dt>
                                    <dd class="mt-0.5 capitalize text-neutral-800">{{ $alert->context ?? '—' }}</dd>
                                </div>
                                <div>
                                    <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Subject type') }}</dt>
                                    <dd class="mt-0.5 truncate text-neutral-800">{{ $alert->subject_type ?? '—' }}</dd>
                                </div>
                                <div>
                                    <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Subject ID') }}</dt>
                                    <dd class="mt-0.5 truncate font-mono text-xs text-neutral-800">{{ $alert->subject_id ?? '—' }}</dd>
                                </div>
                                <div>
                                    <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Resolved by') }}</dt>
                                    <dd class="mt-0.5 truncate text-neutral-800">{{ $alert->resolver?->name ?? '—' }}</dd>
                                </div>
                            </dl>

                            @if (! empty($alert->reasons))
                                <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Reasons') }}</p>
                            ...
                            @if ($alert->resolution_note)
                                <div class="mb-4 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-neutral-700">
                                    <span class="font-semibold">{{ __('Resolution note:') }}</span> {{ $alert->resolution_note }}
                                </div>
                            @endif

                            @if ($alert->case_id)
                                <div class="mb-2 flex items-center justify-between rounded-lg border border-brand-200 bg-brand-50 px-3 py-2">
                                    <span class="text-sm text-neutral-700">{{ __('Linked to a compliance case.') }}</span>
                                    <x-ui.button href="{{ route('admin.compliance', ['tab' => 'cases', 'viewCase' => $alert->case_id]) }}" variant="ghost" size="sm" icon="arrow-right">{{ __('Open case') }}</x-ui.button>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="mt-4 flex justify-end gap-2">
                            @if ($alert->status !== \App\Enums\AlertStatus::Cleared)
                                <x-ui.button type="button" x-on:click="viewingAlert = null; clearingId = '{{ $alert->id }}'" variant="success" size="sm" icon="check">{{ __('Clear') }}</x-ui.button>
                                @if ($alert->status !== \App\Enums\AlertStatus::Escalated)
                                    <form method="POST" action="{{ route('admin.compliance.alert.escalate', $alert->id) }}" onsubmit="return confirm('Escalate this alert?')">
                                        @csrf
                                        <x-ui.button type="submit" variant="danger" size="sm" icon="arrow-trending-up">{{ __('Escalate') }}</x-ui.button>
                                ...
                                @endif
                            @endif
                            <x-ui.button type="button" variant="secondary" x-on:click="viewingAlert = null">{{ __('Close') }}</x-ui.button>
                        </div>
                    </div>
                </div>
            @endforeach
            ...
            @foreach ($cases as $case)
                <div x-show="viewingCase === '{{ $case->id }}'" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative flex max-h-[85vh] w-full max-w-2xl flex-col pp-card p-6" role="dialog" aria-modal="true">
                        <div class="mb-4 flex items-start justify-between">
                            <div>
                                <h3 class="text-lg font-semibold text-neutral-900">{{ $case->reason ?? __('Compliance case') }}</h3>
                        ...
                        <div class="mb-4 flex flex-wrap items-center gap-2">
                            ...
                            @if ($case->sar_filed)<x-ui.badge color="indigo" icon="document-text">{{ __('SAR filed') }}</x-ui.badge>@endif
                            <span class="text-xs text-neutral-400">{{ __('Opened') }} {{ $case->created_at?->diffForHumans() }}</span>
                        </div>

                        <div class="min-h-0 flex-1 overflow-y-auto">
                            <dl class="mb-4 grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                                <div>
                                    <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Assignee') }}</dt>
                                    <dd class="mt-0.5 text-neutral-800">{{ $case->assignee?->name ?? '—' }}</dd>
                                </div>
                                <div>
                                    <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Opened by') }}</dt>
                                    <dd class="mt-0.5 text-neutral-800">{{ $case->opener?->name ?? '—' }}</dd>
                                </div>
                                @if ($case->sar_reference)
                                    <div>
                                        <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('SAR reference') }}</dt>
                                        <dd class="mt-0.5 font-mono text-xs text-neutral-800">{{ $case->sar_reference }}</dd>
                                    </div>
                                @endif
                                @if ($case->closed_at)
                                    <div>
                                        <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Closed') }}</dt>
                            ...
                            @if ($case->summary)
                                <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Summary') }}</p>
                                <p class="mb-4 text-sm text-neutral-700">{{ $case->summary }}</p>
                            @endif

                            @if ($case->resolution)
                                <div class="mb-4 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-neutral-700">
                                    <span class="font-semibold">{{ __('Resolution:') }}</span> {{ $case->resolution }}
                                </div>
                            @endif

                            <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Alerts') }}</p>
                            <x-ui.table :headers="[__('Type'), __('Severity'), __('Status'), __('Date')]">
                                ...
                                @empty
                                    <tr><td colspan="4"><x-ui.empty-state icon="bell-alert" :title="__('No alerts')" :description="__('No alerts linked to this case.')" /></td></tr>
                                @endforelse
                            </x-ui.table>
                        </div>
                        ...
                        <div class="mt-4 flex flex-wrap justify-end gap-2">
                            @if ($case->status !== \App\Enums\CaseStatus::Closed)
                                ...
                                @if ($canSar && ! $case->sar_filed)
                                    <x-ui.button type="button" x-on:click="viewingCase = null; sarId = '{{ $case->id }}'" variant="secondary" size="sm" icon="document-text">{{ __('File SAR') }}</x-ui.button>
                                @endif
                                <x-ui.button type="button" x-on:click="viewingCase = null; closingId = '{{ $case->id }}'" variant="danger" size="sm" icon="lock-closed">{{ __('Close') }}</x-ui.button>
                            @endif
                            <x-ui.button type="button" variant="secondary" x-on:click="viewingCase = null">{{ __('Close') }}</x-ui.button>
                        </div>
                    </div>
                </div>
            @endforeach
