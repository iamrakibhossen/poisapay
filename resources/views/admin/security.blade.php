<x-layouts.admin :title="'Security'">
    <div class="space-y-6">
        <x-ui.page-header title="Security Monitoring" subtitle="Login anomalies, velocity, whitelist blocks, feature flags & audit-chain integrity." />

        {{-- KPIs --}}
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <x-ui.stat-card label="Critical (24h)" :value="number_format($stats['critical_24h'])" icon="exclamation-triangle" accent="rose" />
            <x-ui.stat-card label="New devices (24h)" :value="number_format($stats['new_devices_24h'])" icon="computer-desktop" accent="amber" />
            <x-ui.stat-card label="Sign-ins (24h)" :value="number_format($stats['logins_24h'])" icon="arrow-right-on-rectangle" accent="brand" />
            <div class="pp-card flex items-center justify-between p-5">
                <div>
                    <p class="text-[11px] font-medium uppercase tracking-wide text-gray-400">Audit chain</p>
                    @if ($stats['chain'])
                        <p class="mt-1 text-lg font-semibold {{ $stats['chain']['ok'] ? 'text-emerald-600' : 'text-rose-600' }}">
                            {{ $stats['chain']['ok'] ? 'Intact ('.$stats['chain']['count'].')' : 'BROKEN @'.$stats['chain']['brokenAt'] }}
                        </p>
                    @else
                        <p class="mt-1 text-lg font-semibold text-gray-400">Not verified</p>
                    @endif
                </div>
                <form method="POST" action="{{ route('admin.security.verify-chain') }}">
                    @csrf
                    <x-ui.button type="submit" variant="secondary" size="sm">Verify now</x-ui.button>
                </form>
            </div>
        </div>
        ...
        {{-- Feature flags --}}
        <x-ui.card title="Security modules" subtitle="Toggle detection & enforcement. Changes take effect immediately.">
        ...
        {{-- IP denylist --}}
        <x-ui.card title="IP denylist" subtitle="Sign-ins and reputation checks flag these addresses.">
        ...
        <div>
            <div class="mb-3 flex items-center justify-between">
                <h2 class="text-base font-semibold text-neutral-900">Security events</h2>
                <form method="GET">
            ...
            <x-ui.table :headers="['User', 'Type', 'Severity', 'IP', 'Risk', 'When']">
                @forelse ($events as $e)
                ...
                @empty
                    <tr><td colspan="6"><x-ui.empty-state icon="shield-check" title="No security events" description="Nothing matches this filter." /></td></tr>
                @endforelse
            </x-ui.table>
