<x-layouts.admin :title="'Revenue Transactions'">
    <div class="space-y-6">
        <x-ui.page-header title="Revenue Transactions" subtitle="Every fee credit that makes up the revenue wallet — searchable and exportable.">
            <x-slot:actions>
                <x-ui.button :href="route('admin.revenue-transactions.export', request()->query())" variant="secondary" icon="arrow-down-tray">Export CSV</x-ui.button>
            </x-slot:actions>
        </x-ui.page-header>
        ...
        <form method="GET" action="{{ route('admin.revenue-transactions') }}" class="pp-card p-4">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-6">
                ...
                <x-ui.input label="User" name="user" placeholder="Name or email" icon="magnifying-glass" :value="$user" />
                <x-ui.select label="Fee type" name="feeType">
                ...
                <div class="flex items-end">
                    <x-ui.button :href="route('admin.revenue-transactions')" variant="ghost" icon="x-mark">Reset</x-ui.button>
                </div>
            </div>
        </form>
        ...
        <x-ui.table :headers="['Transaction ID', 'User', 'Fee Type', 'Source', 'Amount', 'Currency', 'Created At']">
            @forelse ($rows as $row)
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td class="px-3 py-3">
                        ...
                        @else
                            <span class="text-xs text-neutral-400">— / system</span>
                        @endif
                    </td>
                    <td class="px-3 py-3">
            ...
            @empty
                <tr><td colspan="7"><x-ui.empty-state icon="banknotes" title="No revenue transactions" description="Fee credits matching your filters will appear here." /></td></tr>
            @endforelse
        </x-ui.table>
