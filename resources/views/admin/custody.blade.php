<x-layouts.admin :title="'Custody & Xpubs'">
    ...
    <div x-data="{
        ...
        <x-ui.page-header title="Custody & Xpubs" subtitle="HD-wallet extended public keys used to derive deposit addresses (custody design §4.2 / D4).">
            <x-slot:actions>
                <x-ui.button x-on:click="create()" icon="plus" size="sm">Register xpub</x-ui.button>
            </x-slot:actions>
        </x-ui.page-header>

        <x-ui.alert type="info" title="Public keys only">
            Register only extended <strong>public</strong> keys (xpub / tpub / ypub / zpub). Private keys (xprv / tprv) are never
            accepted or displayed — they must stay in offline custody. These xpubs let the platform derive fresh deposit
            addresses without ever holding spend authority.
        </x-ui.alert>

        <x-ui.table :headers="['Chain', 'Label', 'Purpose', 'Derivation path', 'Addresses derived', 'Xpub', 'Active', '']">
            @forelse ($xpubs as $x)
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td class="px-4 py-3 text-sm text-neutral-600" title="Monotonic address counter (system-managed)">{{ number_format($x->next_index) }}</td>
                    ...
                    <td class="px-4 py-3">
                        <form method="POST" action="{{ route('admin.custody.toggle', $x->id) }}">
                            ...
                            <button type="submit" class="inline-flex">
                                <x-ui.badge :color="$x->is_active ? 'success' : 'gray'" dot>{{ $x->is_active ? 'Active' : 'Disabled' }}</x-ui.badge>
                            </button>
                        </form>
                    </td>
                    <td class="px-4 py-3 text-right">
                        <div class="flex justify-end gap-2">
                            <x-ui.button variant="secondary" size="sm" icon="pencil-square"
                                x-on:click="edit({{ Illuminate\Support\Js::from(['id' => $x->id, 'chain_id' => $x->chain_id, 'label' => $x->label, 'xpub' => $raw, 'derivation_path' => (string) $x->derivation_path, 'purpose' => $x->purpose, 'is_active' => (bool) $x->is_active]) }})">Edit</x-ui.button>
            ...
            @empty
                <tr><td colspan="8"><x-ui.empty-state icon="key" title="No xpubs registered" description="Register an extended public key to start deriving deposit addresses." /></td></tr>
            @endforelse
        </x-ui.table>
            ...
            <div class="relative w-full max-w-lg pp-card p-6">
                ...
                <form method="POST" action="{{ route('admin.custody.save') }}" class="space-y-4">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <x-ui.select label="Chain" name="chain_id" x-model="form.chain_id" :error="$errors->first('chain_id')">
                            <option value="">Select a chain…</option>
                            @foreach ($chains as $chain)
                                <option value="{{ $chain->id }}">{{ $chain->key->label() }}</option>
                            @endforeach
                        </x-ui.select>
                        <x-ui.input label="Label" name="label" x-model="form.label" placeholder="Hot deposit vault" :error="$errors->first('label')" />
                    </div>
                    <x-ui.textarea label="Extended public key (xpub)" name="xpub" x-model="form.xpub" :rows="3" class="font-mono text-xs" placeholder="xpub6…" :error="$errors->first('xpub')" hint="Public keys only. Must start with xpub, tpub, ypub or zpub." />
                    <div class="grid gap-4 sm:grid-cols-2">
                        <x-ui.input label="Derivation path" name="derivation_path" x-model="form.derivation_path" placeholder="m/44'/60'/0'/0" :error="$errors->first('derivation_path')" />
                        <x-ui.select label="Purpose" name="purpose" x-model="form.purpose" :error="$errors->first('purpose')">
                            <option value="deposit">Deposit (derive addresses)</option>
                            <option value="cold-watch">Cold watch-only</option>
                        </x-ui.select>
                    </div>
