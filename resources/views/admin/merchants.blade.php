<x-layouts.admin :title="__('Merchants')">
    ...
    <div class="space-y-6" x-data="{ viewingId: null, suspendingId: null, feeEditingId: null }">
        <x-ui.page-header :title="__('Merchants')" :subtitle="__('Approve, monitor and suspend merchant accounts.')" />

        {{-- Stat cards --}}
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
            <x-ui.stat-card :label="__('Total merchants')" :value="number_format($stats['total'])" icon="building-storefront" accent="brand" />
            <x-ui.stat-card :label="__('Active')" :value="number_format($stats['active'])" icon="check-circle" accent="emerald" />
            <x-ui.stat-card :label="__('Needs review')" :value="number_format($stats['pending'])" icon="clock" accent="amber" />
            <x-ui.stat-card :label="__('Suspended')" :value="number_format($stats['suspended'])" icon="no-symbol" accent="rose" />
            <x-ui.stat-card :label="__('Paid volume')" :value="number_format($stats['volume'] / 100, 2)" icon="banknotes" accent="brand" />
        </div>
        ...
        <form method="GET" action="{{ route('admin.merchants') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <x-ui.input name="search" :value="$search" icon="magnifying-glass" :placeholder="__('Search business, slug, owner…')" class="w-full sm:w-72" />
        </form>

        <x-ui.table :headers="[__('Business'), __('Owner'), __('Category'), __('Fee'), __('Status'), __('Invoices'), __('Joined'), '']">
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td class="px-3 py-3 text-sm">
                        ...
                        @if ($merchant->fee_bps === null)
                            <span class="ml-1 text-xs text-neutral-400">{{ __('default') }}</span>
                        @endif
                    </td>
                    ...
                    <td class="px-3 py-3 text-sm text-neutral-600">
                        <span class="tabular">{{ number_format($totalCount) }}</span> {{ __('total') }}
                        <span class="block text-xs text-neutral-400">{{ $paidVolume }} {{ __('paid') }}</span>
                    </td>
                        ...
                        <div class="flex items-center justify-end gap-1.5">
                            @if ($canManage)
                                ...
                                @elseif ($merchant->status === \App\Enums\MerchantStatus::Active)
                                    <x-ui.button type="button" x-on:click="suspendingId = '{{ $merchant->id }}'" variant="danger" size="sm" icon="no-symbol">{{ __('Suspend') }}</x-ui.button>
                                @elseif ($merchant->status === \App\Enums\MerchantStatus::Suspended)
                                    <form method="POST" action="{{ route('admin.merchants.reactivate', $merchant->id) }}"
                                        ...
                                        @csrf
                                        <x-ui.button type="submit" variant="secondary" size="sm" icon="arrow-path">{{ __('Reactivate') }}</x-ui.button>
                                    </form>
                                @endif
                                <x-ui.button type="button" x-on:click="feeEditingId = '{{ $merchant->id }}'" variant="ghost" size="sm" icon="adjustments-horizontal">{{ __('Fee') }}</x-ui.button>
                            @endif
                            <x-ui.button type="button" x-on:click="viewingId = '{{ $merchant->id }}'" variant="secondary" size="sm" icon="eye">{{ __('View') }}</x-ui.button>
                        </div>
                    </td>
                </tr>
            @empty
                <tr><td colspan="8"><x-ui.empty-state icon="building-storefront" :title="__('No merchants')" :description="__('No merchant accounts match your filters.')" /></td></tr>
            @endforelse
        </x-ui.table>
        ...
        @foreach ($merchants as $merchant)
            ...
            @if ($canManage)
                <div x-show="suspendingId === '{{ $merchant->id }}'" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative w-full max-w-md pp-card p-6">
                        <div class="mb-4 flex items-start justify-between">
                            <h3 class="text-lg font-semibold text-neutral-900">{{ __('Suspend merchant') }}</h3>
                        ...
                        <p class="mb-4 text-sm text-neutral-500">{{ __('Suspending blocks the merchant from accepting new payments. Provide a reason — it is shown to the merchant.') }}</p>
                        <form method="POST" action="{{ route('admin.merchants.suspend', $merchant->id) }}" class="space-y-4">
                            @csrf
                            <x-ui.textarea :label="__('Reason')" name="suspendReason" :rows="3" :placeholder="__('e.g. Pending compliance review')" :error="$errors->first('suspendReason')" />
                            <div class="flex justify-end gap-2">
                                <x-ui.button type="button" variant="secondary" x-on:click="suspendingId = null">{{ __('Cancel') }}</x-ui.button>
                                <x-ui.button type="submit" variant="danger" icon="no-symbol">{{ __('Suspend') }}</x-ui.button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div x-show="feeEditingId === '{{ $merchant->id }}'" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative w-full max-w-md pp-card p-6">
                        <div class="mb-4 flex items-start justify-between">
                            <h3 class="text-lg font-semibold text-neutral-900">{{ __('Processing fee') }}</h3>
                        ...
                        <p class="mb-4 text-sm text-neutral-500">{{ __('Fee in basis points (0–10000). 100 bps = 1%. Leave blank to reset to the platform default.') }}</p>
                        <form method="POST" action="{{ route('admin.merchants.fee', $merchant->id) }}" class="space-y-4">
                            @csrf
                            <x-ui.input :label="__('Fee (bps)')" name="feeInput" :value="$merchant->fee_bps" type="number" min="0" max="10000" step="1" :placeholder="__('Default')" :error="$errors->first('feeInput')" />
                            <div class="flex justify-end gap-2">
                                <x-ui.button type="button" variant="secondary" x-on:click="feeEditingId = null">{{ __('Cancel') }}</x-ui.button>
                                <x-ui.button type="submit" variant="primary" icon="check">{{ __('Save fee') }}</x-ui.button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div x-show="viewingId === '{{ $merchant->id }}'" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div class="relative flex max-h-[85vh] w-full max-w-2xl flex-col pp-card p-6">
                    ...
                    <div class="mb-4 flex flex-wrap items-center gap-2">
                        ...
                        <span class="tabular text-xs text-neutral-500">{{ __('Fee') }} {{ rtrim(rtrim(number_format($merchant->feeBps() / 100, 2), '0'), '.') }}%</span>
                        @if ($merchant->approved_at)<span class="text-xs text-neutral-400">{{ __('Approved') }} {{ $merchant->approved_at?->diffForHumans() }}</span>@endif
                    </div>

                    @if ($merchant->status === \App\Enums\MerchantStatus::Suspended && $merchant->suspension_reason)
                        <div class="mb-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
                            <span class="font-semibold">{{ __('Suspension reason:') }}</span> {{ $merchant->suspension_reason }}
                        </div>
                    @endif
                    ...
                    <dl class="mb-4 grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Website') }}</dt>
                        ...
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Support email') }}</dt>
                            <dd class="mt-0.5 truncate text-neutral-800">{{ $merchant->support_email ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Statement descriptor') }}</dt>
                            <dd class="mt-0.5 truncate font-mono text-neutral-800">{{ $merchant->statement_descriptor ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Settlement asset') }}</dt>
                    ...
                    @if ($canManage)
                        <div class="mb-4 flex flex-wrap items-center gap-2">
                            ...
                            @elseif ($merchant->status === \App\Enums\MerchantStatus::Active)
                                <x-ui.button type="button" x-on:click="viewingId = null; suspendingId = '{{ $merchant->id }}'" variant="danger" size="sm" icon="no-symbol">{{ __('Suspend') }}</x-ui.button>
                            @elseif ($merchant->status === \App\Enums\MerchantStatus::Suspended)
                                <form method="POST" action="{{ route('admin.merchants.reactivate', $merchant->id) }}"
                                    onsubmit="return confirm('Reactivate this merchant?')">
                                    @csrf
                            ...
                            @endif
                            <x-ui.button type="button" x-on:click="viewingId = null; feeEditingId = '{{ $merchant->id }}'" variant="ghost" size="sm" icon="adjustments-horizontal">{{ __('Set fee') }}</x-ui.button>
                        </div>
                    @endif

                    <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-neutral-500">{{ __('Recent invoices') }}</p>
                    <div class="min-h-0 flex-1 overflow-y-auto">
                        <x-ui.table :headers="[__('Reference'), __('Amount'), __('Fee'), __('Net'), __('Status'), __('Date')]">
                            ...
                            @empty
                                <tr><td colspan="6"><x-ui.empty-state icon="receipt-percent" :title="__('No invoices')" :description="__('This merchant has no invoices yet.')" /></td></tr>
                            @endforelse
                        </x-ui.table>
                    </div>
