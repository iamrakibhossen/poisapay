<x-layouts.admin :title="__('FAQs')">
    ...
    <div x-data="{
        ...
        <x-ui.page-header :title="__('FAQs')" :subtitle="__('Questions answered on the public help page.')">
            <x-slot:actions>
                <x-ui.button x-on:click="create()" icon="plus" size="sm">{{ __('New FAQ') }}</x-ui.button>
            </x-slot:actions>
        </x-ui.page-header>

        <x-ui.table :headers="[__('Question'), __('Group'), __('Homepage'), __('Order'), __('Status'), '']">
            @forelse ($faqs as $faq)
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <x-ui.button variant="secondary" size="sm" icon="pencil-square"
                                x-on:click="edit({{ Illuminate\Support\Js::from(['id' => $faq->id, 'question' => $faq->question, 'answer' => $faq->answer, 'group' => (string) $faq->group, 'sort_order' => (int) $faq->sort_order, 'status' => $faq->status, 'show_on_homepage' => (bool) $faq->show_on_homepage]) }})">{{ __('Edit') }}</x-ui.button>
                            <form method="POST" action="{{ route('admin.faqs.delete', $faq->id) }}" onsubmit="return confirm('{{ __('Delete this FAQ? This cannot be undone.') }}')">
                                @csrf @method('DELETE')
            ...
            @empty
                <tr><td colspan="6"><x-ui.empty-state icon="question-mark-circle" :title="__('No FAQs')" :description="__('Add your first question and answer.')" /></td></tr>
            @endforelse
        </x-ui.table>
        ...
        <div x-show="open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-2xl pp-card p-6 max-h-[90vh] overflow-y-auto">
                ...
                <form method="POST" action="{{ route('admin.faqs.save') }}" class="space-y-4">
                    ...
                    <x-ui.input :label="__('Question')" name="question" x-model="form.question" :placeholder="__('How do I make a deposit?')" :error="$errors->first('question')" />
                    <x-ui.textarea :label="__('Answer')" name="answer" x-model="form.answer" :rows="5" :placeholder="__('Explain the answer clearly.')" :error="$errors->first('answer')" />
                    <div class="grid gap-4 sm:grid-cols-3">
                        <x-ui.input :label="__('Group')" name="group" x-model="form.group" :placeholder="__('Deposits')" :error="$errors->first('group')" />
                        <x-ui.input :label="__('Sort order')" type="number" name="sort_order" x-model="form.sort_order" placeholder="0" :error="$errors->first('sort_order')" />
                        <x-ui.select :label="__('Status')" name="status" x-model="form.status" :error="$errors->first('status')">
                            <option value="published">{{ __('Published') }}</option>
                            <option value="draft">{{ __('Draft') }}</option>
                        </x-ui.select>
                    </div>
                    <x-ui.checkbox name="show_on_homepage" value="1" x-model="form.show_on_homepage" :label="__('Show on homepage')" />
