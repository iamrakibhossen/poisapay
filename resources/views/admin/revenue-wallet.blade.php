<x-layouts.admin :title="'Revenue Wallet'">
    <div class="space-y-6" x-data="{ showWithdraw: {{ $errors->any() ? 'true' : 'false' }} }">
        <x-ui.page-header title="Revenue Wallet" subtitle="Company earnings from fees, card spend and FX spread — derived directly from the ledger.">
            @if ($canWithdraw && $asset)
                <x-slot:actions>
                    <x-ui.button type="button" x-on:click="showWithdraw = true" icon="banknotes">Withdraw</x-ui.button>
                </x-slot:actions>
            @endif
        </x-ui.page-header>

        @if (! $asset)
            <x-ui.empty-state icon="banknotes" title="No asset configured" description="Add a crypto asset (e.g. USDT) to see revenue." />
        @else
            {{-- Balance card --}}
            <div class="pp-card overflow-hidden p-6">
                <div class="flex flex-wrap items-center justify-between gap-6">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-wide text-neutral-500">Available revenue · {{ $asset->symbol }}</p>
                        <p class="tabular mt-2 text-4xl font-bold tracking-tight text-emerald-600">{{ $balance }}</p>
                        <p class="mt-1 text-sm text-neutral-500">Cumulative withdrawn: <span class="tabular font-semibold text-neutral-700">{{ $withdrawn }}</span></p>
                    </div>
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
                <x-ui.stat-card label="Today" :value="$stats['today']->format()" icon="sun" accent="emerald" />
            ...
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
                <div class="pp-card p-5">
                    <h2 class="flex items-center text-md font-semibold text-neutral-900">
                        ...
                        <span>Daily revenue <span class="font-normal text-neutral-400">· last 14 days</span></span>
                    </h2>
                    <div x-data="{
                            init() {
                ...
                <div class="pp-card p-5">
                    <h2 class="flex items-center text-md font-semibold text-neutral-900">
                        ...
                        <span>Monthly revenue <span class="font-normal text-neutral-400">· last 6 months</span></span>
                    </h2>
                    <div x-data="{
                            init() {
                                window.ppChart(this.$refs.canvas, {
            ...
            <div x-show="showWithdraw" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div class="relative w-full max-w-md pp-card p-6">
                    <div class="mb-4 flex items-start justify-between">
                        <h3 class="text-lg font-semibold text-neutral-900">Withdraw {{ $asset->symbol }} revenue</h3>
                    ...
                    <p class="mb-4 text-sm text-neutral-500">Available: <span class="tabular font-semibold text-emerald-600">{{ $available }}</span></p>
                    <form method="POST" action="{{ route('admin.revenue-wallet.withdraw') }}" class="space-y-4">
                        ...
                        <x-ui.select label="Network" name="network" :error="$errors->first('network')">
                        ...
                        <x-ui.input label="Destination address" name="destination" placeholder="0x… / wallet address" :value="old('destination')" :error="$errors->first('destination')" />
                        ...
                        <div class="flex justify-end gap-2 pt-1">
                            <x-ui.button type="button" variant="secondary" x-on:click="showWithdraw = false">Cancel</x-ui.button>
                            <x-ui.button type="submit" icon="banknotes">Request withdrawal</x-ui.button>
                        </div>
                    </form>
                </div>
            </div>
