@php
    ...
    // Workflow-grouped nav (section headings + direct links); a FEW items carry
    ...
    $attention = \App\Support\AdminAttention::counts();

    $admin = auth('admin')->user();
    $can = fn ($perm) => empty($perm) || $admin?->can($perm) || $admin?->hasRole('super-admin');
    // An item is reachable when it has an external URL or its named route is registered.
    // This lets AdminMenu list pages that are still being built without breaking nav.
    $reachable = fn ($item) => ! empty($item['url']) || (! empty($item['route']) && Route::has($item['route']));
    $badge = fn ($item) => ($k = $item['badge'] ?? null) ? (int) ($attention[$k] ?? 0) : 0;
@endphp

@php
    $pill = '<span class="ms-auto inline-flex min-w-[1.25rem] items-center justify-center rounded-full bg-brand-500 px-1.5 py-0.5 text-[0.65rem] font-bold leading-none text-ink-900">%d</span>';
@endphp
        ...
        <div class="px-3 pb-8">
            ...
            <div class="mb-2 flex h-16 items-center justify-center">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-2.5 text-lg font-bold uppercase tracking-wider">
                    <img src="{{ asset('images/logo.svg') }}" alt="PaishaPay" class="h-8 w-8 shrink-0" />
                    <span class="text-white">PaishaPay</span>
                </a>
            </div>
            ...
            <nav class="flex flex-col">
                @foreach ($groups as $group)
                    @php
                        // Keep only items the operator can see AND that resolve (a parent
                        // survives if any child is visible + reachable).
                        $items = collect($group['items'])->filter(function ($item) use ($can, $reachable) {
                            return isset($item['children'])
                                ? collect($item['children'])->contains(fn ($c) => $can($c['perm'] ?? null) && $reachable($c))
                                : $can($item['perm'] ?? null) && $reachable($item);
                        })->all();
                    @endphp
                        ...
                        @else
                            ...
                            <div x-data="{ open: @js($open) }" class="mb-0.5">
                                <button type="button" @click="open = ! open"
                                    ...
                                    @if ($childBadgeSum > 0){!! sprintf($pill, $childBadgeSum) !!}@endif
