...
<div class="sticky top-0 z-10 w-full border-b border-gray-200 bg-white px-4 py-2 lg:px-8">
    <div class="flex h-12 w-full items-center gap-4 lg:gap-5">
        <button @click="sidebarOpen = !sidebarOpen" class="cursor-pointer text-gray-600 hover:text-gray-900" aria-label="{{ __('Toggle sidebar') }}">
        ...
        <div class="ml-auto flex items-center justify-center gap-4">
            ...
            <div class="relative" x-data="{ open: false }" @keydown.escape="open = false">
                ...
                <div x-show="open" x-cloak @click.outside="open = false" x-transition.origin.top.right
                    ...
                    <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3">
                        ...
                        @if ($unread > 0)
                            <form method="POST" action="{{ route('admin.notifications.read-all') }}">
                                @csrf
                                <button type="submit" class="text-xs font-medium text-blue-600 hover:text-blue-800">{{ __('Mark all read') }}</button>
                            </form>
                        @endif
                    </div>
                    <div class="max-h-96 overflow-y-auto">
                            ...
                            <form method="POST" action="{{ route('admin.notifications.read', $n->id) }}" class="block">
                                ...
                                <button type="submit"
                                    ...
                                    <span class="block min-w-0 flex-1">
                                        <span class="block truncate text-sm font-medium text-gray-800">{{ $n->data['title'] ?? __('Notification') }}</span>
                        ...
                        @empty
                            <div class="px-4 py-10 text-center text-sm text-gray-500">{{ __('No notifications yet') }}</div>
                        @endforelse
                    </div>
                    <a href="{{ route('admin.notifications') }}" class="block border-t border-gray-200 px-4 py-3 text-center text-sm font-medium text-gray-600 hover:bg-gray-50">{{ __('View all') }}</a>
                </div>
            </div>
            ...
            <div class="relative" x-data="{ open: false }" @keydown.escape="open = false">
                ...
                <div x-show="open" x-cloak x-transition @click.outside="open = false"
                    ...
                    <a href="{{ route('home') }}" class="flex items-center gap-2 px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-100"><x-heroicon-o-globe-alt class="h-5 w-5" /> {{ __('View site') }}</a>
                    <form method="POST" action="{{ route('admin.logout') }}">
                        @csrf
                        <button type="submit" class="flex w-full items-center gap-2 px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-100"><x-heroicon-o-arrow-right-start-on-rectangle class="h-5 w-5" /> {{ __('Logout') }}</button>
                    </form>
                </div>
            </div>
            @endauth
