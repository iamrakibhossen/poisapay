<x-layouts.app :title="__('Send Money')">
    <div class="mx-auto mt-6 max-w-lg space-y-6">
        <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-500">{{ __('Instant, zero-fee transfers to any PoisaPay user.') }}</p>
                </div>
            </div>
            @if ($recentCount > 0)
                <a href="{{ route('send.history') }}" class="group inline-flex shrink-0 items-center gap-1 text-sm font-medium text-neutral-500 transition hover:text-brand-600">
                    {{ __('View all') }}
        ...
        <x-ui.card>
            @if (! count($wallets))
                <x-ui.empty-state icon="wallet" :title="__('No funds to send')"
                    :description="__('Deposit or top up a balance before you can send money.')">
                    <x-slot:action>
            ...
            @else
                ...
                <form method="POST" action="{{ route('send.execute') }}" class="space-y-5" x-on:submit="submitting = true"
                    ...
                    {{-- Recipient --}}
                    <div>
                        <label class="pp-label" for="recipient">{{ __('Recipient') }}</label>
                        <div class="relative">
                    ...
                    <div class="rounded-2xl border p-4 transition" :class="insufficient ? 'border-rose-300 bg-rose-50/40' : 'border-neutral-200 bg-neutral-50/60'">
                        <div class="mb-2 flex items-center justify-between">
                            <span class="text-xs font-medium text-neutral-500">{{ __('You send') }}</span>
                            <button type="button" x-show="selected" x-on:click="setMax()" class="text-xs font-medium text-neutral-500 transition hover:text-brand-700">
                                {{ __('Balance:') }} <span class="tabular font-semibold text-neutral-700" x-text="selected?.availableFormatted"></span>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="relative flex shrink-0 items-center gap-2 rounded-full bg-white py-1.5 pl-1.5 pr-8 shadow-sm ring-1 ring-neutral-200 transition hover:ring-neutral-300">
                                ...
                                <select x-model.number="assetId" class="absolute inset-0 cursor-pointer opacity-0" aria-label="{{ __('Asset') }}">
                        ...
                        <div class="mt-1.5 text-xs">
                            <span x-show="insufficient" x-cloak class="font-medium text-rose-600">{{ __('More than your available balance.') }}</span>
                    ...
                    <x-ui.input :label="__('Memo (optional)')" name="memo" icon="chat-bubble-left-ellipsis" :value="old('memo')"
                        :placeholder="__('What\'s this for?')" :error="$errors->first('memo')" />
                    ...
                    <div x-show="valid" x-cloak class="space-y-2 rounded-xl border border-neutral-200 bg-neutral-50 p-4 text-sm">
                        <div class="flex items-center justify-between text-neutral-600">
                            ...
                            <span class="font-semibold text-emerald-600">{{ __('Free') }}</span>
                        </div>
                    ...
                    <x-ui.button type="submit" class="w-full" icon="paper-airplane" x-bind:disabled="!valid || submitting">
                        <span x-show="!submitting"><span x-show="valid">{{ __('Send') }} <span class="tabular" x-text="fmt(amountNum)"></span> <span x-text="selected?.symbol"></span></span><span x-show="!valid">{{ __('Send now') }}</span></span>
