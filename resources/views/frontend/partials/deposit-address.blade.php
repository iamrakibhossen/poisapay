...
<div>
    @if ($addressError)
        <x-ui.alert type="danger" title="Address unavailable">{{ $addressError }}</x-ui.alert>
    @elseif ($address)
        <div class="flex flex-col items-center gap-5 sm:flex-row sm:items-start" x-data="{ copied: false }">
            ...
            <div class="w-full min-w-0 flex-1">
                <label class="pp-label">{{ $addressNetwork }} address</label>
                ...
                <p class="mt-2 text-xs text-neutral-500">
                    Send only {{ $selectedAsset->symbol }} on the {{ $addressNetwork }} network.
                    Deposits are credited after <span class="font-medium text-neutral-700">{{ $selectedAsset->requiredConfirmations() }}</span> network confirmations.
                </p>
            </div>
        </div>
    @endif
