...
{{-- Shareable PaishaPay ID — hand this number out to get paid. --}}
<div class="flex items-center justify-between rounded-xl border border-neutral-200 bg-neutral-50/60 px-4 py-3">
    <div class="min-w-0">
        <p class="text-xs font-medium text-neutral-500">{{ __('Your PaishaPay ID') }}</p>
...
<x-settings.section :title="__('Personal details')" :description="__('Your name and how we reach you.')">
    <form method="POST" action="{{ route('settings.profile') }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <x-ui.select :label="__('Base currency')" name="baseCurrency" :error="$errors->first('baseCurrency')">
            @foreach (\App\Support\BaseCurrency::options() as $code => $label)
