...
<x-settings.section title="Devices" description="Devices that have signed in to your account.">
        ...
        @forelse ($devices as $d)
            <div class="flex items-center gap-3 rounded-xl border p-3 {{ $d['current'] ? 'border-emerald-200 bg-emerald-50/40' : 'border-neutral-200' }}">
                ...
                <div class="min-w-0 flex-1">
                    <p class="truncate text-sm font-medium text-neutral-900">{{ $d['name'] ?? 'Unknown device' }}</p>
                    <p class="truncate text-xs text-neutral-500">{{ $d['ip'] ?? 'Unknown IP' }} · {{ $d['last'] ?? '—' }}</p>
                </div>
                @if ($d['current'])
                    <span class="shrink-0 rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs font-semibold text-emerald-700">This device</span>
                @else
                    <form method="POST" action="{{ route('settings.device.revoke', $d['id']) }}" onsubmit="return confirm('Revoke this device?')">
                        @csrf @method('DELETE')
                        <x-ui.button type="submit" variant="ghost" size="sm" icon="trash">Revoke</x-ui.button>
                    </form>
                @endif
            </div>
        @empty
            <x-ui.empty-state icon="computer-desktop" title="No devices yet"
