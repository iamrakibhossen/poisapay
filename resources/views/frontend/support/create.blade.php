<x-layouts.app :title="'New ticket'">
    <div class="mx-auto max-w-2xl">
        <header class="mb-6">
            ...
            <h1 class="mt-2 text-2xl font-semibold tracking-tight text-neutral-900">New ticket</h1>
        </header>
        ...
        <form method="POST" action="{{ route('support.store') }}" class="space-y-4 rounded-2xl border border-neutral-200 bg-white p-6">
            @csrf
            <div>
                <label class="mb-1 block text-sm font-medium text-neutral-700">Subject</label>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-neutral-700">Priority</label>
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-neutral-700">How can we help?</label>
                <textarea name="body" required rows="6" maxlength="5000" class="w-full rounded-lg border-neutral-300 text-sm">{{ old('body') }}</textarea>
            </div>
            <button class="rounded-lg bg-neutral-900 px-4 py-2 text-sm font-semibold text-white">Submit ticket</button>
        </form>
    </div>
</x-layouts.app>
