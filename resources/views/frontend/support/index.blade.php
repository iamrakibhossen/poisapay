<x-layouts.app :title="'Support'">
    <div class="mx-auto max-w-4xl">
        <header class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold tracking-tight text-neutral-900">Support</h1>
                <p class="mt-1 text-sm text-neutral-500">Get help from our team.</p>
            </div>
            <a href="{{ route('support.create') }}" class="rounded-lg bg-neutral-900 px-4 py-2 text-sm font-semibold text-white">New ticket</a>
        </header>
        ...
        <div class="divide-y divide-neutral-100 rounded-2xl border border-neutral-200 bg-white">
            @forelse ($tickets as $ticket)
                <a href="{{ route('support.show', $ticket->id) }}" class="flex items-center justify-between px-5 py-4 hover:bg-neutral-50">
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-neutral-500">{{ ucfirst($ticket->category) }} · {{ $ticket->messages_count }} message{{ $ticket->messages_count === 1 ? '' : 's' }} · {{ $ticket->updated_at->diffForHumans() }}</p>
                    </div>
                    <span @class([
            ...
            @empty
                <p class="px-5 py-10 text-center text-sm text-neutral-400">No tickets yet. Open one if you need help.</p>
            @endforelse
        </div>
