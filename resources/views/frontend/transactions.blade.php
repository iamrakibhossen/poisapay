<x-layouts.app :title="'Transactions'">
    <div class="space-y-5">
        <h1 class="text-xl font-semibold tracking-tight text-neutral-900">Transactions</h1>

        {{-- Stats --}}
        <div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
            @php
                $stats = [
                    ['label' => 'All-time', 'value' => number_format($feed['total']), 'icon' => 'clock', 'fg' => 'text-neutral-900'],
                    ['label' => 'This month', 'value' => number_format($feed['month_count']), 'icon' => 'calendar-days', 'fg' => 'text-neutral-900'],
                    ['label' => 'Received · 30d', 'value' => $analytics['inflow'], 'icon' => 'arrow-down-left', 'fg' => 'text-emerald-600'],
                    ['label' => 'Sent · 30d', 'value' => $analytics['outflow'], 'icon' => 'arrow-up-right', 'fg' => 'text-neutral-900'],
                ];
            @endphp
            @foreach ($stats as $s)
        ...
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center">
            <div class="-mx-1 flex flex-nowrap gap-1 overflow-x-auto px-1 lg:flex-wrap">
                @foreach (['all' => 'All', 'deposits' => 'Deposits', 'withdrawals' => 'Withdrawals', 'transfers' => 'Transfers', 'swaps' => 'Swaps', 'payments' => 'Payments', 'cards' => 'Cards'] as $key => $label)
            ...
            <form method="GET" action="{{ route('transactions') }}" class="flex gap-2 lg:ml-auto">
                ...
                <div class="relative flex-1 lg:w-56 lg:flex-none">
                    ...
                    <input type="search" name="search" value="{{ $filters['search'] }}" placeholder="Search…" class="pp-input w-full !pl-10 text-sm" />
                </div>
            </form>
        </div>
        ...
        @else
            <div class="pp-card">
                <x-ui.empty-state icon="banknotes" title="No transactions" description="Nothing matches your filters yet." />
            </div>
        @endif
        ...
        @if ($feed['last_page'] > 1)
            <div class="flex items-center justify-between text-sm">
                ...
                @endif
                <span class="text-neutral-500">Page {{ $feed['page'] }} of {{ $feed['last_page'] }}</span>
