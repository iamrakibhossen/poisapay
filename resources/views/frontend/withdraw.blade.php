<x-layouts.app :title="__('Withdraw')">
    <div class="mx-auto max-w-2xl space-y-6">
        ...
        @unless ($enabled)
            <x-ui.alert type="warning" :title="__('Withdrawals temporarily disabled')">
                {{ __('Withdrawals are paused for maintenance right now. Please check back shortly.') }}
            </x-ui.alert>
        @endunless
        ...
        @if ($enabled)
            @php
                ...
                if ($fiatDetail) {
                    ...
                    $stepLabels = [__('Asset'), __('Network'), __('Confirm')]; $currentStep = 2;
                } else {
            ...
            <div>
                <x-ui.card>
                            ...
                            <div x-data="{
                                ...
                                <div class="mb-5 flex items-center gap-3 rounded-xl bg-emerald-50/60 p-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="text-sm font-semibold text-neutral-900">{{ __('Local Withdraw') }} · {{ $fiatDetail['symbol'] }}</p>
                                        <p class="text-xs text-neutral-500">{{ __('Available') }} <span class="tabular font-medium text-neutral-700">{{ $fiatDetail['available'] }}</span></p>
                                    </div>
                                    <a href="{{ route('withdraw.index') }}" class="text-xs font-medium text-brand-600 hover:text-brand-700">{{ __('Change') }}</a>
                                </div>
                                    ...
                                    <form method="POST" action="{{ route('withdraw.fiat') }}" class="space-y-5" x-on:submit="submitting = true">
                                        ...
                                        <div>
                                            <div class="flex items-center justify-between">
                                                <label class="pp-label" for="fiatAmount">{{ __('Amount') }}</label>
                                                <span class="text-xs text-neutral-500">{{ __('Max') }} <span class="tabular font-medium text-neutral-700" x-text="fmt(max)"></span></span>
                                            </div>
                                            <div class="relative">
                                            ...
                                            <div class="mt-1.5 text-xs">
                                                <span x-show="exceeds" x-cloak class="font-medium text-red-500">{{ __('Exceeds your available balance after fees.') }}</span>
                                                <span x-show="!exceeds && belowMin" x-cloak class="font-medium text-red-500">{{ __('Below the minimum') }} (<span x-text="method?.min"></span>).</span>
                                                <span x-show="!exceeds && !belowMin && aboveMax" x-cloak class="font-medium text-red-500">{{ __('Above the maximum') }} (<span x-text="method?.max"></span>).</span>
                                                <span x-show="!exceeds && !belowMin && !aboveMax && method" class="text-neutral-400">
                                                    {{ __('Fee') }} <span x-text="method?.feeLabel"></span> · {{ __('Min') }} <span x-text="method?.min"></span><template x-if="method?.max"> · {{ __('Max') }} <span x-text="method.max"></span></template>
                                                </span>
                                        ...
                                        @if (count($fiatDetail['accounts']))
                                            <div>
                                                <label class="pp-label">{{ __('Pay out to') }}</label>
                                                <div class="space-y-2">
                                                    @foreach ($fiatDetail['accounts'] as $acct)
                                                        <label class="group flex cursor-pointer items-center gap-3 rounded-xl border p-3 transition"
                                                            ...
                                                            <button type="button" form="delacct-{{ $acct['id'] }}" class="text-neutral-300 hover:text-red-500" title="{{ __('Remove') }}">
                                                    ...
                                                    <label class="flex cursor-pointer items-center gap-3 rounded-xl border p-3 transition"
                                                        ...
                                                        <span class="text-sm font-medium text-neutral-900">{{ __('Use a new account') }}</span>
                                                    </label>
                                                </div>
                                            </div>
                                        @endif
                                        ...
                                        <div x-show="isNew" x-cloak class="space-y-4">
                                            <div>
                                                <x-ui.select :label="__('Payout method')" name="methodId" x-model="methodId" :error="$errors->first('methodId')">
                                                    @foreach ($fiatDetail['methods'] as $m)
                                                        <option value="{{ $m['id'] }}">{{ $m['name'] }} ({{ __(ucfirst($m['type'])) }})</option>
                                                    @endforeach
                                                </x-ui.select>
                                            </div>
                                            ...
                                            <div x-show="method && method.isBank" x-cloak>
                                                <x-ui.input :label="__('Bank name')" name="bankName" :value="old('bankName')" :placeholder="__('e.g. BRAC Bank')" :error="$errors->first('bankName')" />
                                            </div>

                                            <x-ui.input :label="__('Account holder name')" name="accountName" :value="old('accountName')" icon="user" :placeholder="__('As registered')" :error="$errors->first('accountName')" />
                                            <div>
                                                <label class="pp-label" x-text="method ? method.numberLabel : '{{ __('Account number') }}'">{{ __('Account number') }}</label>
                                                <x-ui.input name="accountNumber" :value="old('accountNumber')" icon="hashtag" :placeholder="__('Account or mobile number')" :error="$errors->first('accountNumber')" />
                                            </div>
                                            ...
                                            <label class="flex items-center gap-2 text-sm text-neutral-700">
                                                ...
                                                {{ __('Save this account for next time') }}
                                            </label>
                                            <div x-show="save" x-cloak>
                                                <x-ui.input name="label" :value="old('label')" :placeholder="__('Label (e.g. My bKash)')" :error="$errors->first('label')" />
                                            </div>
                                        </div>
                                        ...
                                        @if ($requires2fa)
                                            <x-ui.input :label="__('Two-factor code')" name="twoFactorCode" icon="shield-check"
                        ...
                        @elseif ($coins->isEmpty() && $cashOptions->isEmpty())
                            <x-ui.empty-state icon="wallet" :title="__('Nothing to withdraw')"
                                :description="__('You need a funded balance before you can withdraw.')">
                                <x-slot:action>
                        ...
                        @elseif (! $coin)
                            ...
                            @if ($coins->isNotEmpty())
                                <p class="mb-3 text-sm font-semibold text-neutral-900">{{ __('Crypto Withdraw') }} <span class="font-normal text-neutral-400">· {{ __('to an external wallet') }}</span></p>
                                    ...
                                    @foreach ($coins as $c)
                                        @php
                                            ...
                                            $params = $c['networkCount'] === 1
                                                ...
                                                : ['coin' => $c['symbol']];
                                        @endphp
                                        <a href="{{ route('withdraw.index', $params) }}"
                                            ...
                                            <div class="text-right">
                                                ...
                                                <p class="text-[11px] text-neutral-400">{{ $c['networkCount'] }} {{ $c['networkCount'] === 1 ? __('network') : __('networks') }}</p>
                                            </div>
                        ...
                        @elseif (! $assetId || ! $networkDetail)
                            ...
                            <div class="mb-4 flex items-center justify-between">
                                <p class="text-sm font-semibold text-neutral-900">{{ __('Choose the :coin network', ['coin' => $coin]) }}</p>
                                <a href="{{ route('withdraw.index') }}" class="inline-flex items-center gap-1 text-xs font-medium text-brand-600 hover:text-brand-700">
                                    <x-heroicon-o-arrow-left class="h-3.5 w-3.5" /> {{ __('Change coin') }}
                                </a>
                            </div>
                            <x-ui.alert type="warning" class="mb-4">
                                {{ __('Send to the matching network only. Withdrawing on the wrong network can permanently lose funds.') }}
                            </x-ui.alert>
                            <div class="space-y-2.5">
                                @foreach ($selectedCoin['networks'] as $n)
                                    <a href="{{ route('withdraw.index', ['coin' => $coin, 'asset' => $n['assetId']]) }}"
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <p class="text-xs text-neutral-500">{{ $coin }} {{ __('network') }}</p>
                                        </div>
                            ...
                            <div x-data="{
                                ...
                                <div class="mb-5 flex items-center gap-3 rounded-xl bg-neutral-50 p-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="text-xs text-neutral-500">{{ $networkDetail['network'] }} {{ __('network') }}</p>
                                    </div>
                                    <a href="{{ route('withdraw.index') }}" class="text-xs font-medium text-brand-600 hover:text-brand-700">{{ __('Change') }}</a>
                                </div>
                                ...
                                <form method="POST" action="{{ route('withdraw.submit') }}" class="space-y-5" x-on:submit="submitting = true">
                                    ...
                                    <div>
                                        <div class="flex items-center justify-between">
                                            <label class="pp-label" for="amount">{{ __('Amount') }}</label>
                                            <span class="text-xs text-neutral-500">{{ __('Available') }} <span class="tabular font-medium text-neutral-700">{{ $networkDetail['available'] }}</span></span>
                                        </div>
                                        <div class="relative">
                                        ...
                                        <div class="mt-1.5 flex items-center justify-between text-xs">
                                            <span x-show="exceeds" x-cloak class="font-medium text-red-500">{{ __('Exceeds your available balance after fees.') }}</span>
                                            <span x-show="!exceeds" class="text-neutral-400">{{ __('Max withdrawable:') }} <span class="tabular" x-text="fmt(max)"></span> {{ $coin }}</span>
                                            @error('amount')<span class="font-medium text-red-500">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                    ...
                                    {{-- Destination address --}}
                                    <div>
                                        <label class="pp-label" for="toAddress">{{ __('Destination address') }}</label>
                                        <div class="relative">
                                            <input id="toAddress" name="toAddress" x-model="address" type="text"
                                                placeholder="{{ __('Recipient wallet address') }}" autocomplete="off" spellcheck="false"
                                        ...
                                        <p class="mt-1.5 text-xs text-neutral-400">{{ __("Double-check the address — on-chain withdrawals to the wrong address can't be reversed.") }}</p>
                                    </div>

                                    {{-- Memo --}}
                                    <x-ui.input :label="__('Memo (optional)')" name="memo" icon="chat-bubble-left-ellipsis" :value="old('memo')"
                                        :placeholder="__('Internal note (not sent on-chain)')" :error="$errors->first('memo')" />
                                    ...
                                    <div class="space-y-2 rounded-xl border border-neutral-200 bg-neutral-50 p-4 text-sm">
                                        ...
                                        <div class="flex justify-between border-t border-neutral-200 pt-2 font-semibold text-neutral-900">
                                            <span>{{ __('Total debited') }}</span><span class="tabular" x-bind:class="exceeds ? 'text-red-500' : ''"><span x-text="fmt(totalDebited)"></span> {{ $coin }}</span>
                                        </div>
                                    </div>
                                    ...
                                    @if ($requires2fa)
                                        <x-ui.input :label="__('Two-factor code')" name="twoFactorCode" icon="shield-check"
                                            ...
                                            :hint="__('Enter the 6-digit code from your authenticator app.')" />
                                    @else
                                        <x-ui.alert type="info">
                                            {{ __('Tip: enable two-factor authentication to add an extra layer of protection to withdrawals.') }}
                                        </x-ui.alert>
                                    @endif

                                    <x-ui.button type="submit" class="w-full" icon="arrow-up-tray"
                                        x-bind:disabled="!valid || submitting">
                                        <span x-show="!submitting"><span x-show="hasAmount && !exceeds">{{ __('Withdraw') }} <span class="tabular" x-text="fmt(amountNum)"></span> {{ $coin }}</span><span x-show="!hasAmount || exceeds">{{ __('Withdraw') }}</span></span>
                                        <span x-show="submitting" x-cloak>{{ __('Processing…') }}</span>
                                    </x-ui.button>
                                </form>
                            </div>
