<x-layouts.app :title="'Exchange'">
    ...
    <div class="space-y-6">
        <div class="mx-auto max-w-lg text-center">
            <h1 class="text-2xl font-semibold tracking-tight text-neutral-900">Exchange</h1>
            <p class="mt-1 text-sm text-neutral-500">Swap between coins at a live reference rate.</p>
            @if ($recentCount > 0)
                <a href="{{ route('swaps') }}" class="group mt-3 inline-flex items-center gap-1 text-sm font-medium text-neutral-500 transition hover:text-brand-600">
                    View swap history
        ...
        <div class="mx-auto max-w-lg">
            @if ($coins->count() < 2)
                <x-ui.card>
                    <x-ui.empty-state icon="arrows-right-left" title="Exchange unavailable"
                        description="No coins are available to swap right now." />
                </x-ui.card>
            @else
                <x-ui.card>
                    <form method="POST" action="{{ route('exchange.quote') }}" class="space-y-4"
                        ...
                        <div class="rounded-2xl border p-4 transition" :class="insufficient ? 'border-rose-300 bg-rose-50/40' : 'border-neutral-200 bg-neutral-50/60'">
                            <div class="mb-2 flex items-center justify-between">
                                <span class="text-xs font-medium text-neutral-500">You pay</span>
                                <button type="button" x-show="fromBalance" x-on:click="setMax()" class="text-xs font-medium text-neutral-500 transition hover:text-brand-700">
                                    Balance: <span class="tabular font-semibold text-neutral-700" x-text="fromBalance?.formatted"></span>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="relative flex shrink-0 items-center gap-2 rounded-full bg-white py-1.5 pl-1.5 pr-8 shadow-sm ring-1 ring-neutral-200 transition hover:ring-neutral-300">
                                    ...
                                    <select x-model.number="fromAssetId" class="absolute inset-0 cursor-pointer opacity-0" aria-label="Pay with">
                            ...
                            <p x-show="insufficient" x-cloak class="mt-1.5 flex items-center gap-1 text-xs font-medium text-rose-600">
                                <x-heroicon-o-exclamation-triangle class="h-3.5 w-3.5" /> More than your available balance.
                            </p>
                        </div>

                        {{-- Swap direction --}}
                        <div class="-my-3 flex justify-center">
                            <button type="button" x-on:click="swap()" title="Switch coins"
                        ...
                        <div class="rounded-2xl border border-neutral-200 bg-neutral-50/60 p-4">
                            <div class="mb-2 flex items-center justify-between">
                                <span class="text-xs font-medium text-neutral-500">You receive <span class="text-neutral-400">(estimated)</span></span>
                            </div>
                            <div class="flex items-center gap-3">
                                ...
                                <div class="relative flex shrink-0 items-center gap-2 rounded-full bg-white py-1.5 pl-1.5 pr-8 shadow-sm ring-1 ring-neutral-200 transition hover:ring-neutral-300">
                                    ...
                                    <x-heroicon-o-chevron-down class="pointer-events-none absolute right-2.5 h-4 w-4 text-neutral-400" />
                                    <select x-model.number="toAssetId" class="absolute inset-0 cursor-pointer opacity-0" aria-label="Receive">
                            ...
                            <p x-show="sameAsset" x-cloak class="mt-1.5 text-xs font-medium text-rose-600">Choose two different coins.</p>
                        ...
                        @unless ($activeQuote)
                            <x-ui.button type="submit" size="lg" class="w-full" icon="calculator" x-bind:disabled="! canGetQuote">
                                <span x-show="! insufficient">Get quote</span>
                                <span x-show="insufficient" x-cloak>Insufficient balance</span>
                            </x-ui.button>
                        @endunless
                    </form>
                            ...
                            <div class="space-y-2 rounded-xl border border-neutral-200 bg-neutral-50 p-4 text-sm">
                                ...
                                <div class="flex items-center justify-between border-t border-neutral-200 pt-2 text-neutral-600">
                                    ...
                                    <span class="tabular inline-flex items-center gap-1 font-semibold" :class="remaining > 0 ? 'text-amber-700' : 'text-rose-600'">
                                        ...
                                        <span x-show="remaining <= 0" x-cloak>Expired</span>
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="space-y-2">
                                <form method="POST" action="{{ route('exchange.confirm') }}">
                                    ...
                                    <x-ui.button type="submit" variant="success" class="w-full" icon="check"
                                        x-bind:disabled="remaining <= 0">Confirm swap</x-ui.button>
                                </form>
                                ...
                                <form method="POST" action="{{ route('exchange.quote') }}">
                                    ...
                                    <button type="submit" class="w-full text-center text-xs font-medium text-neutral-500 hover:text-amber-700">Refresh quote</button>
                                </form>
                            </div>
                        </div>
                    @endif
